# forecast stock returns - prototyping version for testing of changes
# notes and commentary ----
# try to predict returns 21-days forward. consider papers:
# https://jfin-swufe.springeropen.com/articles/10.1186/s40854-024-00644-0
# https://www.sciencedirect.com/science/article/pii/S2666827025000143
# https://drive.google.com/file/d/1uvjBJ9D09T0_sp7kQppWpD-xelJ0KQhc/edit
# https://www.dynotrading.com/10-best-macd-settings-for-effective-trading/

# ideas:
# nested forecasting: https://business-science.github.io/modeltime/articles/nested-forecasting.html
# tabpfn: https://tidyverse.org/blog/2026/03/tabpfn-0-1-0/

# 0.0 Setup ----
# * packages ----
# Time Series ML
library(tidymodels)
library(modeltime)
library(modeltime.ensemble)
library(modeltime.resample)
library(finetune)
library(xgboost)
library(lightgbm)
library(prophet)
library(bonsai)
library(catboost)
# library(boostime)
# library(finnts)

# Timing & Parallel Processing
library(tictoc)
library(future)
library(future.apply)
library(doFuture)
# library(future.mirai)

# Core 
library(tidyquant)
library(tidyverse)
library(timetk)
library(data.table)
library(plotly)
library(rstatix)
library(tidytable)

# Modeling and stock analytics
# library(baguette)
library(PortfolioAnalytics)
library(shapviz)
library(ggthemes)
# library(rvest) # for read_html()
library(TTR)
library(xts)
library(shapviz)

# * dates ----
# set dates to allow enough time to generate all of the rolling metrics which may require more than 1 year of data
from      <- today() - years(4)# - months(1)
train_date <- today() - years(2)#-months(6)

# * Helping functions ----
# * Calibrate function ----
calibrate_and_plot <- function(..., type = 'testing', plot = T){
    
    if (type == 'testing'){
        new_data = testing(splits)
    } else {
        new_data = training(splits)
    }
    
    calibration_tbl <- modeltime_table(...) %>% 
        modeltime_calibrate(new_data)
    
    print(calibration_tbl %>% modeltime_accuracy())
    
    if (plot == T){calibration_tbl %>% 
            modeltime_forecast(
                new_data = new_data,
                actual_data = data_prepared_tbl
            ) %>% 
            plot_modeltime_forecast(.conf_interval_show = F)
    } else {
        return()
    }
}

# * plot forecast function ----
plot_fcst <- function(forecast, scales = "fixed", ncol = 3){
    g <- forecast %>% 
        group_by(symbol) %>% 
        ggplot(aes(x = date, y = .value, color = .model_desc))+
        geom_line()+
        geom_hline(yintercept = 0)+
        facet_wrap(~symbol, ncol = ncol, scales = scales)+
        scale_x_date(date_breaks = "6 months", date_labels =  "%m-%Y")+
        theme_bw()+
        theme(legend.position = "bottom",
              strip.background = element_rect("#0076BA"),
              strip.text = element_text(color = "white"),
              axis.text.x=element_text(angle=60, hjust=1))+
        scale_color_tq()+
        ylab("")
    
    ggplotly(g)
}

# * Forecast win rate ----
winrate <- function(forecast, model = "model"){
    wins <- forecast %>%
        filter(.key == "prediction") %>% 
        select(.index, symbol, .value, Return_fwd_21) %>% 
        rename(prediction = .value, 
               actual     = Return_fwd_21) %>% 
        mutate(pred_gain   = if_else(prediction > 0, 1, 0),
               actual_gain = if_else(actual > 0, 1, 0),
               win         = if_else(pred_gain == actual_gain, 1, 0),
               pred_fact   = as.factor(pred_gain),
               actual_fact = as.factor(actual_gain)) %>% 
        mutate(model = model)
    
    setDT(wins) 
}


# 1.0 Data ----
# * symbols ----
# SP500
sp500 <- tq_index("SP500") #tq_index() options include "DOW"       "DOWGLOBAL" "SP400"     "SP500"     "SP600" 
# sp400 <- tq_index("SP400")
# sp600 <- tq_index("SP600")

# could also use tq_exchange("NYSE"), tq_exchange("AMEX") tq_exchange("NASDAQ")

exclude_symbols <- c("TSLA","PLTR")

sp500_symbols <- sp500 %>% 
    filter(symbol != "-" & !str_detect(company,"CL C")) %>% 
    filter(symbol %notin% exclude_symbols) %>% 
    slice_max(weight, n = 350) %>%
    # slice_sample(prop = 0.1) %>%
    arrange(symbol) %>% 
    pull(symbol) 

sp400_symbols <- sp400 %>%
    filter(symbol != "-" & !str_detect(company,"CL C")) %>%
    filter(symbol %notin% exclude_symbols) %>%
    slice_max(weight, n = 50) %>%
    # slice_sample(prop = 0.1) %>%
    arrange(symbol) %>%
    pull(symbol)

# if we want to test a smaller set for exploration
# set.seed(27)
# rand_symbols <- slice_sample(sp500,prop = 0.15) %>% filter(symbol != "-" & !str_detect(company,"CL C")) %>% pull(symbol)

# * symbols for testing ----
symbols <- c("CNP","MPC","CMS","EXC","MU","STX", # # test for 2 symbols with median rmse, two best, two worst from february forecast
             "NVDA","AAPL","NWS","PSKY", # 2 highest and lowest weights of sp 500
             "KR","PFG","ACN","MMM","HST","FAST","GM", # some randoms
             "LITE", "COHR", "FTI", # top 3 weights from SP400
             "SW"
             ) # sw has a lot of 0 and should get filtered out

symbols <- c("MDLZ","AAPL","TRGP","GD","ETR","JPM","SCHW","WFC","ISRG","ACN")

# symbols_all <- union(rand_symbols, symbols)

# ** symbols by exchange ----
exchanges <- bind_rows(tq_exchange("NYSE"),tq_exchange("NASDAQ"))
setDT(exchanges)

exchanges[, country := as.factor(country)]

exchanges %>% summary()

exchanges %>% plot_ly(y = ~log(market.cap), x = ~fct_reorder(country, market.cap, .desc = T), type = "box")

symbols <- exchanges %>% 
    filter(country == "United States") %>% 
    filter(symbol != "-" & !str_detect(company,"Class C")) %>% 
    filter(symbol %notin% exclude_symbols) %>% 
    dplyr::slice_max(order_by = market.cap, n = 500)

symbols %>% filter(symbol %notin% sp500$symbol) %>% View()

# * get stock price data ----
prices_base <- tq_get(symbols, from = from) 

setDT(prices_base)

# ** view data ----
prices_base

prices_base[,.(.N),keyby = symbol] %>% arrange(N)
prices_base[,(uniqueN(symbol))]

# * reduce data with filters ----
# check for any duplicates by date. yahoo can sometimes do this
# create a count of symbols by day, then count each observation within the day
prices_base[,N := .N, keyby = .(symbol,date)][,day_count := 1:.N, keyby = .(symbol,date,N)]

# keep only the unique daily value or the second if there are 2 on a day
prices_base <- prices_base[N == 1 | N == 2 & day_count == 2][,!c("N","day_count")]



# must still be active at time of modeling with minimum 3 years data
# minimum close on most recent day > $10
symbols_filtered_list <- prices_base[!is.na(close),
                                     .(count = .N,
                                       max = max(date),
                                       min = min(date)),
                                     keyby = symbol][count >= 252*3 & max == max(max),
                                                     symbol]

symbols_filtered_list <- symbols_filtered_list[!symbols_filtered_list %in%
                           prices_base[date == max(date),.(symbol,close)][close <= 10,symbol]]

symbols_filtered_list <- symbols_filtered_list[!symbols_filtered_list %in%
                                                   prices_base[volume == 0,.N, keyby = symbol][N > 1,symbol]]


# * create filtered dataset ----
prices_dt <- prices_base[symbol %in% symbols_filtered_list &
                             !is.na(close),]

# Engineered indicators ----
# https://blog.elearnmarkets.com/best-25-technical-indicators/

# * function for features ----
add_features <- function(prices_dt, price) {
  price_col <- prices_dt[[price]]
  
  # Calculate Indicators
  prices_dt[, Close_macd_long := MACD(price_col, 63, 252, 21)[, "macd"]]
  prices_dt[, Close_macd_long_signal := MACD(price_col, 63, 252, 21)[, "signal"]]
  prices_dt[, Close_macd_short := MACD(price_col, 18, 36, 18, list(list(EMA, wilder=TRUE),list(EMA, wilder=TRUE),list(EMA, wilder=TRUE)))[, "macd"]]
  prices_dt[, Close_macd_short_signal := MACD(price_col, 18, 36, 18, list(list(EMA, wilder=TRUE),list(EMA, wilder=TRUE),list(EMA, wilder=TRUE)))[, "signal"]]
  # prices_dt[, Close_macd_short := MACD(price_col, 12, 26, 9, list(list(EMA, wilder=TRUE),list(EMA, wilder=TRUE),list(EMA, wilder=TRUE)))[, "macd"]]
  # prices_dt[, Close_macd_short_signal := MACD(price_col, 12, 26, 9, list(list(EMA, wilder=TRUE),list(EMA, wilder=TRUE),list(EMA, wilder=TRUE)))[, "signal"]]
  prices_dt[, ":=" (
      Close_macd_long_trend           = frollmean(Close_macd_long,63),
      Close_macd_long_signal_trend    = frollmean(Close_macd_long_signal,63),
      Close_macd_short_trend          = frollmean(Close_macd_short,63),
      Close_macd_short_signal_trend   = frollmean(Close_macd_short_signal,63),
      Close_macd_long_trading_signal  = Close_macd_long-Close_macd_long_signal,
      Close_macd_short_trading_signal = Close_macd_short-Close_macd_short_signal
      )]
  prices_dt[, Close_ema_50_ratio := price_col/EMA(price_col, n = 50)]
  prices_dt[, Close_ema_20_ratio := price_col/EMA(price_col, n = 21)]
  prices_dt[, Close_ppo_line_12_26 := ((EMA(price_col, 12) - EMA(price_col, 26)) / EMA(price_col, 26)) * 100]
  # prices_dt[, Close_oscillator_7_14_28 := ultimateOscillator(prices_dt[, .(high, low, price_col)])]
  prices_dt[, Close_roc_0_1      := ROC(price_col, n = 1)]
  # prices_dt[, Close_roc_0_1_roll := frollmean(Close_roc_0_1, n=21, align = "right", na.rm = T)]
  prices_dt[, Close_roc_short    := ROC(price_col, n = 21, "discrete")]
  prices_dt[, Close_roc_long     := ROC(price_col, n = 126, "discrete")]
  prices_dt[, Close_roc_0_1_std_63 := frollsd(Close_roc_0_1, 63, fill = NA, align = "right")]
  # prices_dt[, Close_natr_63      := ATR(prices_dt[,.(high, low, price_col)], n = 63)[,"atr"]/price_col]
  prices_dt[, Close_ATR_63       := ATR(prices_dt[,.(high, low, price_col)], n = 63)[,"atr"]]
  # prices_dt[, Close_rsi_10       := RSI(price_col, n = 10)]
  prices_dt[, Close_RSI_14       := RSI(price_col, n = 14)]
  prices_dt[, Close_cmo_28       := CMO(price_col, n = 28)]
  prices_dt[, Close_cmo_ma       := EMA(Close_cmo_28, n = 21)]
  prices_dt[, Close_cmo_signal   := Close_cmo_28/Close_cmo_ma]
  # prices_dt[, Close_rolling_126_norm := (price_col - frollmean(price_col, 126)) / frollsd(price_col, 126, align = "right")] #used to be standardize vec
  # prices_dt[, Close_rolling_std_126 := frollsd(price_col, 126, align = "right")]
  
  # prices_dt[, Close_SNR_21       := SNR(prices_dt[, .(high, low, price_col)], n = 21)]
  prices_dt[, Close_rel_volatility := 100 - 100 / (1 + runSD(price_col, 10))]
  prices_dt[, Close_252_max_diff := price_col/frollmax(price_col, 252, align = 'right')]
  prices_dt[, Close_252_min_diff := price_col/frollmin(price_col, 252, align = 'right')]
  prices_dt[, Close_CCI          := CCI(prices_dt[, .(high, low, price_col)])]
  prices_dt[, Close_kst          := KST(price_col)[,"kst"]]
  prices_dt[, Close_kst_signal   := KST(price_col)[,"signal"]]
  prices_dt[, Close_TDI          := TDI(price_col, n = 20, multiple = 2)[,"tdi"]]
  prices_dt[, Close_TDI_di       := TDI(price_col, n = 20, multiple = 2)[,"di"]]
  # prices_dt[, Close_TRIX         := TRIX(price_col, n = 20, nSig = 9, "EMA", percent = TRUE)[,"TRIX"]]
  prices_dt[, Close_TRIX_signal  := TRIX(price_col, n = 20, nSig = 9, "EMA", percent = TRUE)[,"signal"]]
  prices_dt[, SAR                := SAR(prices_dt[, .(high, low)])]
  prices_dt[, SAR_ratio          := SAR/close]
  prices_dt[, SAR_ratio_std      := (SAR - frollmean(SAR, 63))/frollsd(SAR, 63)]
  prices_dt[, SMI                := SMI(prices_dt[, .(high, low, price_col)])[,"SMI"]]
  prices_dt[, SMI_signal         := SMI(prices_dt[, .(high, low, price_col)])[,"signal"]]
  prices_dt[, CMF                := CMF(prices_dt[, .(high, low, price_col)], volume)]
  # prices_dt[, EMV                := EMV(prices_dt[, .(high, low)], volume)[,"emv"]]
  prices_dt[, maEMV              := EMV(prices_dt[, .(high, low)], volume)[,"maEMV"]]

  prices_dt[, Close_ADX_DIp      := ADX(prices_dt[,.(high, low, price_col)])[,"DIp"]]
  prices_dt[, Close_ADX_DIn      := ADX(prices_dt[,.(high, low, price_col)])[,"DIn"]]
  prices_dt[, Close_ADX_DX       := ADX(prices_dt[,.(high, low, price_col)])[,"DX"]]
  prices_dt[, Close_ADX_ADX      := ADX(prices_dt[,.(high, low, price_col)])[,"ADX"]]
  
  # relative vigor index
  prices_dt[,":=" (numSMA = SMA(price_col - open, n = 10), denSMA = SMA(high - low, n = 10))][,":=" (rvi = numSMA/denSMA, rvi_signal = SMA(numSMA/denSMA, 4))]
  prices_dt[,":=" (numSMA = NULL, denSMA = NULL)]
  
  # create indicators for intraday effects and day return
  # remove intraday are low importance intraday = high/low-1,
  # prices_dt[, ":=" (HLC_ratio = (high-low)/close
  #                   #OC_ratio  = (open-close)/close
  #                   )]
  
  # Volume-based indicators
  # prices_dt[, Vol_ema_21_norm  := EMA(volume, n = 21) / volume]
  prices_dt[, Vol_roc_0_1      := ROC(volume, n = 1)] # leave this one for other calculations
  prices_dt[, Vol_roc_0_1_std_63 := frollsd(Vol_roc_0_1, 63, align = "right")]
  
  # create indicators based on VWAP
  prices_dt[, Vol_WAP           := VWAP(price_col, volume)] # only used for ratios, then dropped
  # prices_dt[, Vol_WAP_log       := log(Vol_WAP)]
  prices_dt[, Vol_WAP_Close     := Vol_WAP/price_col]
  prices_dt[, Vol_WAP_EMA_ratio := Vol_WAP/EMA(Vol_WAP, n = 21)] # only used for ratios, then dropped
  # prices_dt[, Vol_WAP_EMA_norm  := standardize_vec(EMA(Vol_WAP, n = 21))]
  # prices_dt[, Vol_WAP_EMA_ratio := Vol_WAP/Vol_WAP_EMA]
  prices_dt[, Vol_WAP_ROC       := ROC(Vol_WAP, n = 21)]
  
  # standardize VWAP by 252 days
  prices_dt[, Vol_WAP_norm      := (Vol_WAP - frollmean(Vol_WAP, n = 252, align = "right"))/frollsd(Vol_WAP, n = 252, align = "right")]
  # prices_dt[, Vol_WAP_roll_ratio := Vol_WAP/Vol_WAP_norm_roll]
  
  prices_dt[, ':=' (Vol_WAP = NULL 
                    #,Vol_WAP_EMA = NULL
                    )]
  
  #prices_dt[, Vol_OBV_standard := standardize_vec(OBV(price_col, volume))]
  prices_dt[, Vol_OBV       := OBV(price_col, volume)]
  prices_dt[, Vol_OBV_std   := (Vol_OBV - frollmean(Vol_OBV, n = 126, align = 'right'))/frollsd(Vol_OBV, n = 126, align = 'right')]
  prices_dt[, Vol_OBV_trend := ROC(Vol_OBV, n = 10)]
  prices_dt[, Vol_OBV       := NULL]
  
  prices_dt[, Vol_MFI_21 := MFI(prices_dt[,.(high, low, close)], volume, n = 21)]
  
  # Momentum
  prices_dt[, `:=`(
    Close_lag_1   = data.table::shift(price_col, n = 1),
    Close_lag_21  = data.table::shift(price_col, n = 21),
    Close_lag_252 = data.table::shift(price_col, n = 252)
  )]
  
  prices_dt[, Close_momentum_21_252_126 := ((Close_lag_21 / Close_lag_252 - 1) - (Close_lag_1 / Close_lag_21 - 1))/frollsd(Close_roc_0_1, 126, align = "right")]

  # Z-score features
  prices_dt[, Close_zscore_126 := (price_col - frollmean(price_col, 126, align = "right")) / frollsd(price_col, 126, align = "right")]
  
  prices_dt[, ":=" (Close_roc_0_1 = NULL, 
                    Vol_roc_0_1   = NULL)]
  
  # Efficiently compute lead values & forward returns without loops
  T <- c(5, 10, 21)
  lead_cols   <- paste0("Close_lead_", T)
  return_cols <- paste0("Return_fwd_", T)
  
  prices_dt[, (lead_cols) := lapply(T, function(t) data.table::shift(price_col, -t))]
  prices_dt[, (return_cols) := lapply(lead_cols, function(col) (get(col) / price_col) - 1)]
  
  return(prices_dt)
}

# * Apply function ----
prices_features_list <- future_lapply(split(prices_dt, by = "symbol"), 
                                      add_features,
                                      price = "close")

# Combine the results back into a `data.table`
prices_features_dt <- rbindlist(prices_features_list, use.names = TRUE, fill = TRUE)

# ** Charts ----
# max return by symbol
prices_features_dt[ , .SD[which.max(Return_fwd_21)], by = symbol]

# consider correlations between features
prices_features_dt %>% 
    slice_sample(prop = 0.4) %>% 
  select(where(is.numeric), -(open:adjusted), -contains("_lag_"),
         -contains("_lead_"),
         -Return_fwd_5, -Return_fwd_10) %>% 
  cor_test(Return_fwd_21) %>% 
  filter(var1 != var2) %>% 
  plot_ly(x = ~cor,
          y = ~fct_reorder(var2, cor, .desc = F),
          type = "bar",
          alpha = 0.7) %>% layout(yaxis = list(title = ''))

# library(GGally)
# 
# ggpairs(prices_features_tbl %>% 
#           select(where(is.numeric), 
#                  -(open:adjusted), 
#                  -contains("_lag_"),
#                  -contains("_lead_")),
#   lower = list(
#     continuous = "smooth"
#   )) + theme_tq()

# chart price features to see if they need to be scaled
prices_features_dt %>% 
    filter(symbol %in% symbols_filtered_list[c(7:17)]) %>% droplevels() %>% 
  select(symbol, starts_with("Close_"),  -contains("_lag_"),
         -contains("_lead_")) %>% 
  pivot_longer(-symbol) %>% 
  filter(!is.na(value)) %>% 
  ggplot(aes(y = value, x = symbol, fill = symbol, color = symbol))+
  geom_boxplot(alpha = 0.7, show.legend = F)+
  facet_wrap(~name, scales = "free_y")+
  theme_clean()+
  theme(axis.text.x = element_text(angle = 30, hjust = 1),
        strip.background = element_rect("#0076BA"),
        strip.text = element_text(color = "white"))

prices_features_dt %>% 
    #filter(symbol %in% symbols_filtered_list[c(7:17)]) %>% droplevels() %>% 
    select(symbol, contains("macd")) %>% 
    pivot_longer(-symbol) %>% 
    filter(!is.na(value)) %>% 
    ggplot(aes(y = value, x = symbol, fill = symbol, color = symbol))+
    geom_boxplot(alpha = 0.7, show.legend = F)+
    facet_wrap(~name, scales = "free_y")+
    theme_clean()+
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          strip.background = element_rect("#0076BA"),
          strip.text = element_text(color = "white"))


prices_features_dt %>% 
    #filter(symbol %in% symbols_filtered_list[c(7:17)]) %>% droplevels() %>% 
    select(symbol, contains("macd"), Return_fwd_21) %>% 
    cor_test(Return_fwd_21) %>% 
    arrange(desc(cor))

prices_features_dt %>% 
    filter(symbol %in% symbols[c(7:25)]) %>% droplevels() %>% 
    select(-starts_with("Close_"),  -contains("_lag_"),
           -contains("_lead_"), -starts_with("Return"), -where(is.Date)) %>% 
    pivot_longer(-symbol) %>% 
    filter(!is.na(value)) %>% 
    ggplot(aes(y = value, x = symbol, fill = symbol, color = symbol))+
    geom_boxplot(alpha = 0.7, show.legend = F)+
    facet_wrap(~name, scales = "free_y")+
    theme_clean()+
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          strip.background = element_rect("#0076BA"),
          strip.text = element_text(color = "white"))

prices_features_dt %>% 
    filter(symbol %in% symbols[c(7:27)]) %>% droplevels() %>% 
  select(symbol, starts_with("Vol_")) %>% 
  pivot_longer(-symbol) %>% 
  filter(!is.na(value)) %>% 
  ggplot(aes(y = value, x = symbol, fill = symbol, color = symbol))+
  geom_boxplot(alpha = 0.7, show.legend = F)+
  facet_wrap(~name, scales = "free_y")+
  theme_clean()+
  theme(axis.text.x = element_text(angle = 30, hjust = 1),
        strip.background = element_rect("#0076BA"),
        strip.text = element_text(color = "white"))

prices_features_dt[symbol %in% symbols[c(1:30)],] %>% 
    plot_ly(x = ~Close_kst, y = ~Return_fwd_21, type = 'scatter', mode = 'markers')

prices_features_dt[symbol %in% symbols[c(1:30)],][,.(sar = standardize_vec(SAR),Return_fwd_21), keyby = .(symbol)] %>% 
    plot_ly(x = ~sar, y = ~Return_fwd_21, 
            type = 'scatter', mode = 'markers',
            color = ~symbol)

prices_features_dt %>% 
    filter(symbol %in% symbols[c(1:50)]) %>% droplevels() %>% 
    select(symbol, SAR, Close_kst, Close_kst_signal) %>% 
    pivot_longer(-symbol) %>% 
    filter(!is.na(value)) %>% 
    ggplot(aes(y = value, x = symbol, fill = symbol, color = symbol))+
    geom_boxplot(alpha = 0.7, show.legend = F)+
    facet_wrap(~name, scales = "free_y")+
    theme_clean()+
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          strip.background = element_rect("#0076BA"),
          strip.text = element_text(color = "white"))

prices_features_dt %>% 
    filter(symbol %in% symbols[c(1:5)]) %>% droplevels() %>% 
    select(symbol,Return_fwd_21, contains("ADX")) %>% 
    pivot_longer(-(symbol:Return_fwd_21)) %>% 
    filter(!is.na(value)) %>% 
    ggplot(aes(y = Return_fwd_21, x = value, color = name))+
    geom_point(alpha = 0.7, show.legend = F)+
    geom_smooth(method = "gam", show.legend = F, color = "gray80")+
    facet_wrap(~name, scales = "free_y")+
    theme_clean()+
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          strip.background = element_rect("#0076BA"),
          strip.text = element_text(color = "white"))

prices_features_tbl %>% 
    select(symbol, date, adjusted) %>% 
    pivot_longer(adjusted) %>% 
    plot_time_series(date, value, .color_var = name, 
                     .smooth = F, .facet_vars = symbol,
                     .facet_ncol = 3)

prices_features_dt %>% 
  plot_ly(y = ~Return_fwd_21, x = ~Close_macd_short, type = "scatter")

data_prepared_dt_filter %>% 
    select(-rowid, -symbol, -date) %>% 
    drop_na() %>% 
    slice_sample(prop = 0.5) %>% 
    cor_test() %>% #filter(var1 == "Return_fwd_21" & var1 != var2) %>% arrange(desc(abs(cor)))
    plot_ly(x = ~var1,
            y = ~var2,
            z = ~cor,
            type = "heatmap",
            colors = c("darkred","white","darkgreen"))

# Dividends ----
# * function to get multiple dividends data ----
dividends_tbl <- map(symbols_filtered_list, 
                     ~ getDividends(.x, 
                                    from = floor_date(from, 'quarter')) |> 
                         fortify.zoo(melt  = TRUE, 
                                     names = c("date", "symbol", "dividend")) |>
                         mutate(symbol = str_remove(symbol,".div"))) %>% 
    list_rbind()

setDT(dividends_tbl)

# setnames(dividends_tbl, "name","symbol")

setorderv(dividends_tbl, cols = c("symbol", "date"))

# modify dates tp prevent dividends on a weekend
dividends_tbl[, date := fcase(wday(date) == 7, date + days(2),
                              wday(date) == 1, date + days(1),
                              default = date)]

dividends_tbl[, div_roc := ROC(dividend,1), keyby = symbol]
dividends_tbl[, number_dividends := 1:.N, keyby = symbol]

# * merge dividends to prices ----
prices_div_dt <- merge(
    prices_features_dt,
    dividends_tbl,
    by = c("symbol","date"),
    all.x = T
)

prices_div_dt[, c("dividend",'div_roc',"number_dividends") := lapply(.SD, function(x) nafill(x, "locf")), 
              .SDcols = c("dividend",'div_roc',"number_dividends"), 
              by = symbol]

prices_div_dt[,":=" (div_ann_days = round(as.numeric(difftime(date,
                                                              min(date),
                                                              units = "days")),0)),
              keyby = .(symbol, number_dividends)]

#prices_div_dt[, dividend_std := standardize_vec(dividend), keyby = symbol]

prices_div_dt[, div_close_ratio := dividend / close]

# * fill dividends ----
#setnafill(prices_div_dt,  type = 'const', fill = 0, cols = colnames(prices_div_dt[,.SD, .SDcols = sapply(prices_div_dt, is.numeric)]))
setnafill(prices_div_dt,  type = 'const', fill = 0, cols = colnames(prices_div_dt[,.SD, .SDcols = patterns("div")]))

# ** dividend charts ----
dividends_tbl[,.(.N, avg_dividend = mean(dividend)), keyby = symbol] %>% 
    plot_ly(x = ~avg_dividend, y = ~fct_reorder(symbol, avg_dividend), type = 'bar') %>% 
    layout(yaxis = list(title = ''))

dividends_tbl %>% 
    plot_ly(x = ~number_dividends, y = ~dividend, type = 'scatter')

# dividends_tbl[symbol == 'TDG',]

# Earnings data ----

# * function for earnings for multiple symbols ----

get_earnings_data <- function(symbol){
    
    #symbol <- gsub(x=symbol, pattern="\\-", replacement="\\.")
    
    symbol <- gsub("[/-]", ".", symbol)
    
    
    # create link with the symbol and read the page
    link <- str_glue("https://www.alphaquery.com/stock/{symbol}/earnings-history")
    
    earnings_page <- read_html(link)
    
    #earnings_page
    
    # convert the table to a data.table. works on the alphaquery not the zacks
   earnings_table <- html_element(earnings_page, "table") %>% 
        html_table(header = T, trim = T) %>% 
        setDT()
    
    # format text to dates and numbers
    earnings_table[,':=' (symbol = symbol,
                          date = as.Date(`Announcement Date`),
                          est_eps = str_remove(`Estimated EPS`,pattern = "\\$") %>% as.numeric(),
                          act_eps = str_remove(`Actual EPS`,pattern = "\\$") %>% as.numeric())]
    
    # shift saturday by 2 days, sunday by 1 day
    earnings_table[,date := fcase(wday(date) == 7, date + days(2),
                                  wday(date) == 1, date + days(1),
                                  default = date)]
    
    # for cases where an estimate eps wasn't given, use the actual eps
    earnings_table[,est_eps := fifelse(is.na(est_eps) & !is.na(act_eps), act_eps, est_eps)]
    
    #earnings_table <- earnings_table[date >= from-months(3),]
    
    # arrange by date ascending to allow for date calculations
    setorder(earnings_table, date)
    
    # engineer features in eps
    # use absolutes to avoid divide by 0 and other issues
    # eps_surprise shows how much higher or lower the eps was than the estimate
    # eps growth is the change in actual EPS between announcements
    # ann_number splits the announcements this is used later for calculating the days between announcements
    earnings_table[,":=" (eps_surprise = act_eps-est_eps,
                          eps_growth = act_eps-data.table::shift(act_eps, 1, type = "lag"),
                          #announcement = "Y",
                          num_ann = 1:.N)]
    
    earnings_table <- earnings_table[,select(.SD, -(`Announcement Date`:`Actual EPS`))]
    earnings_table[, symbol := gsub("\\.", "-", symbol)]
}

earning_symbols <- unique(prices_features_dt$symbol)
#earning_symbols_s <- earning_symbols[104:108]

# Use lapply to apply the function to each symbol
earnings_data_list <- future_lapply(earning_symbols,
                                    get_earnings_data)

# Combine the results into a single data.table (if applicable)
earnings_data <- rbindlist(earnings_data_list, fill = TRUE)

setkey(earnings_data, symbol, date)

# * merge eps to prices ----
# prices_fill_tbl %>% count(symbol)

# this creates NAs where there were no earnings
# we will fill these in later

prices_earnings_tbl <- merge(prices_div_dt,
                             earnings_data,
                             by = c("date","symbol"),
                             all.x = T)

setorderv(prices_earnings_tbl, cols = c("symbol","date"))

# ** test on eps ----
prices_earnings_tbl %>% 
    filter(!is.na(est_eps))

prices_earnings_tbl %>% 
  filter(!is.na(est_eps) & symbol == "NVDA" )

prices_earnings_tbl %>% 
    filter(is.na(est_eps))

prices_earnings_tbl %>% 
    filter(symbol == "WBA" & date == "2015-09-08")

# * fill earnings ----
# discount the earnings later by the time with an interaction term
fill_cols <- patterns("eps|num_ann", cols = colnames(prices_earnings_tbl))

prices_earnings_tbl[, (fill_cols) := lapply(.SD, function(x) nafill(x, "locf")), 
                    .SDcols = fill_cols, 
                    by = symbol]

setnafill(prices_earnings_tbl, type = "const", 0, cols = "num_ann")

# this is used later as an interaction variable to see if the time since the announcement affects prediction
prices_earnings_tbl[,":=" (eps_ann_days = round(as.numeric(difftime(date,
                                                               min(date),
                                                               units = "days")),0),
                           pe_ratio = act_eps/close),
                    keyby = .(symbol, num_ann)]

summary(prices_earnings_tbl)

# Commodities ----
commodities <- c(
    "CL=F" # crude oil futures
    #,"GC=F" #gold futures
)
commodity_prices <- tq_get(commodities, from = from-30)

setDT(commodity_prices)
setorderv(commodity_prices, c("symbol","date"))
setnafill(commodity_prices, type = "locf", cols = colnames(commodity_prices[,.SD,.SDcols = sapply(commodity_prices, is.numeric)]))

commodity_prices[, symbol := str_replace_all(symbol,
                                        c("CL=F" = "crude",
                                          "GC=F" = "gold"))]

commodity_prices[,":=" (close_roc_5 = close/data.table::shift(close, 5)-1,
                        close_roc_21 = close/data.table::shift(close, 21)-1,
                        close_max = close/frollmax(close, 252, align = "right"),
                        close_min = close/frollmin(close, 252, align = "right"),
                        close_norm = (close - frollmean(close, 252))/frollsd(close,252)),
                 keyby = .(symbol)]

commodity_prices[, ":=" (
    close_macd = EMA(close, 12) - EMA(close, 26)
    ), 
    keyby = symbol][,":=" (close_macd_signal = EMA(close_macd, 9)), 
                    keyby = symbol]

cast_cols <- colnames(commodity_prices[,.SD, .SDcols = patterns("close")])

commodity_prices_cast <- commodity_prices[,dcast.data.table(.SD, date ~ symbol, value.var = cast_cols)]

commodity_prices_short <- commodity_prices[,!c("open","high","low","adjusted","volume","symbol")]
names = names(commodity_prices_short)
names = names[names != "date"]

setnames(commodity_prices_short, old = names, new = paste0(names, "_oil"))

# * merge to prices data ----
prices_comm_tbl <- merge(prices_earnings_tbl, 
                         commodity_prices_cast, 
                         all.x = T,
                         by = "date")

prices_comm_dt <- merge(prices_features_dt, 
                         commodity_prices_short, 
                         all.x = T,
                         by = "date")

setnames(commodity_prices, "close","close_oil")

# test with just the closing price of oil
prices_comm_dt <- merge(prices_features_dt, 
                        commodity_prices[,.(date, close_oil)], 
                        all.x = T,
                        by = "date")

prices_comm_dt[,":=" (Close_ratio_oil = close/close_oil)]


# SP500 prices ----
sp_dt <- tq_get("^SPX", from = from) %>% 
    setDT()

# Hidden Markov model ----
# * function ----
library(fHMM)

hidden_markov <- function(dt){
  
  data <- dt
  
  # 2 state model
  controls <- set_controls(
    states      = 3,
    sdds        = "t",
    file        = data,
    date_column = "date",
    data_column = "close",
    # file        = as.data.frame(prices_base),,
    logreturns  = TRUE, # converts returns to log if not already
    # from        = "2010-01-01",
    # to          = "2024-12-01",
    runs        = 70
  )
  
  data_hmm <- fHMM::prepare_data(controls) # parsnip also has a prepare_data function
  
  #summary(data_2)
  
  model_hmm <- fit_model(data_hmm, seed = 101, ncluster = 4)
  model_hmm <- decode_states(model_hmm)
  
  state <- model_hmm$decoding
}

# * HMM for SP 500 ----
sp_dt[, sp_state := hidden_markov(.SD), .SDcols = c("date", "close")]

# * apply to prices ----
# start <- Sys.time()
# prices_earnings_tbl[, state := hidden_markov(.SD), by = symbol, .SDcols = c("date", "close")]
# 
# end <- Sys.time()
# end-start
# analysis on returns and 
sp_dt[,":=" (return_sp_21 = data.table::shift(close, -21)/close-1,
             return_sp_1  = data.table::shift(close, -1)/close-1)]

# spy_dt %>% group_by(as.factor(spy_state)) %>% get_summary_stats(return_spy_21)

sp_dt[,.(.N, 
          avg = mean(return_sp_21, na.rm = T), 
          sd  = sd(return_sp_21, na.rm = T), 
          sum = sum(log1p(return_sp_21), na.rm = T)), 
       sp_state]

# * merge SPY HMM to base data ----
prices_earnings_tbl <- merge(prices_earnings_tbl,
                             spy_dt[,.(date, spy_state)], #return_spy_1, return_spy_21, 
                             all.x = T,
                             by = "date")

# Macroeconomic data ----
# * load macroeconomic data ----

# start the from data 3 months earlier to try to get an announcement before
# the start of the prices data
# source for money flow divergence https://www.tradingview.com/script/WUtLXuxM-Money-Flow-Divergence/
indicators <- c("GDPC1", # real gdp
                "CPILFESL", # core cpi # https://fred.stlouisfed.org/series/CPILFESL
                "UNRATE",   # unemployment rate
                # "EMRATIO",  # employment to population ratio
                "FEDFUNDS",  # fed funds effective rate
                "UMCSENT", # u michigan consumer sentiment
                #"DFF", # fed funds daily effective rate
                "M2SL" # M2 money supply for divergence
                # ,"DCOILBRENTEU" # Brent oil daily Europe
                # ,"BBKMGDP"
)

# consider:
# WEI #https://fred.stlouisfed.org/series/WEI
# https://fred.stlouisfed.org/series/BBKMGDP

economic_data <- tq_get(indicators, get = "economic.data", from = from-days(90))
setDT(economic_data)

economic_data[,.N, keyby = symbol]

# * data.table transformations ----
# change name of price because I don't like it
setnames(economic_data, "price","value")

# create the change indicator. some are already % so use an absolute change
economic_data[,":=" (change = value-data.table::shift(value, 1, "lag")), keyby = symbol]

# cast long to wide
economic_data_cast <- economic_data[,dcast(.SD,date ~ symbol, value.var = c("value","change"))]

# add annoucement number. this is useful for splitting and making date calculations from the
# last announcement
economic_data_cast[,econ_ann := 1:.N]

# pad so that we can fill before joining later
# pad to today so when it joins there is a full data set
economic_data_pad <- economic_data_cast %>% 
  pad_by_time(date, 
              .by = "day", 
              .fill_na_direction = "down", 
              .end_date = today()) %>% 
  setDT()

# --- could add a 12-month change here for selected dates?
# change_year := value-data.table::shift(value, 1, "lag")), keyby = symbol

# calculate dates from last announcement
economic_data_pad[,econ_ann_days := round(as.numeric(difftime(date,
                                                              min(date),
                                                              units = "days")),0),
                  keyby = .(econ_ann)]  

# economic_data_pad
economic_data_final <- economic_data_pad[,.(date, 
                                            value_FEDFUNDS,
                                            # value_DFF,
                                            value_UNRATE,
                                            # value_BBKMGDP,
                                            # value_EMRATIO,
                                            value_UMCSENT,
                                            # change_FEDFUNDS,
                                            # change_DFF,
                                            change_UNRATE,
                                            change_CPILFESL, 
                                            change_GDPC1,
                                            change_M2SL,
                                            change_UMCSENT,
                                            # change_EMRATIO,
                                            econ_ann_days)]

#economic_data_final[,unrate_over := fifelse(value_UNRATE > frollmean(value_UNRATE, n = 300),1,0)]

# * join economic data to prices ----
prices_earnings_econ_tbl <- merge(prices_comm_tbl, 
                                  economic_data_final, 
                                  all.x = T,
                                  by = "date")

# ** recreate money flow to sp500 indicator ----
# sp_dt[, change_sp500 := close/shift(close, 1)-1]
# economic_data_cast[,change_M2SL := value_M2SL/shift(value_M2SL,1)-1]
# 
# mfi_dt <- merge(sp_dt[,.(date,change_sp500)], economic_data_cast[,.(date,change_M2SL)], all.x = T)
# 
# setnafill(mfi_dt, type = "locf",cols = "change_M2SL")
# 
# mfi_dt[,mf_divergence := change_M2SL - change_sp500]
# 
# # merge to econ tbl
# prices_earnings_econ_tbl <- merge(prices_earnings_econ_tbl, mfi_dt[,!c("change_M2SL")], by = "date")

# Charts ----
prices_earnings_econ_tbl %>% 
  select(Return_fwd_21, contains("eps"), 
         starts_with("value"),
         starts_with("change")) %>% 
  cor_test() %>% 
  filter(var1 != var2) %>% 
  plot_ly(z = ~cor,
          x = ~var1,
          y = ~var2,
          #color = ~var1,
          type = "heatmap",
          alpha = 0.7) %>% layout(yaxis = list(title = ''))

prices_earnings_econ_tbl %>% 
  filter(symbol == "NVDA") %>% 
  plot_ly(x = ~date,
          y = ~Return_fwd_21,
          color = ~unrate_over,
          type = "scatter")

prices_earnings_econ_tbl %>% 
    filter(symbol == "NVDA") %>% 
    plot_ly(x = ~Close_macd_long_trend,
            y = ~Return_fwd_21,
            color = ~unrate_over,
            type = "scatter")

prices_features_tbl %>% 
  select(symbol, contains("eps")) %>% 
  pivot_longer(-symbol) %>% 
  filter(!is.na(value)) %>% 
  ggplot(aes(y = value, x = symbol, fill = symbol, color = symbol))+
  geom_boxplot(alpha = 0.7, show.legend = F)+
  facet_wrap(~name, scales = "free_y")+
  theme_clean()+
  theme(axis.text.x = element_text(angle = 30, hjust = 1),
        strip.background = element_rect("#0076BA"),
        strip.text = element_text(color = "white"))

prices_base %>% 
    select(date, adjusted, close) %>% 
    pivot_longer(-date) %>% 
    plot_time_series(date, value, .color_var = name, .smooth = F)

prices_base %>%
    ggplot(aes(x = date, y = adjusted)) +
    #geom_barchart(aes(open = open, high = high, low = low, close = close)) +
    geom_line()+
    geom_ma(color = "darkgreen", n = 200, linewidth = 1) +
    geom_ma(color = "red", n = 50, linewidth = 1) +
    geom_ma(color = "mediumblue", ma_fun = EMA, n = 50, linewidth = 1)+
    geom_ma(aes(volume = volume), color = "orange",ma_fun = EVWMA, n = 50, linewidth = 1)+
    coord_x_date(xlim = c("2020-01-01", "2024-01-31"))+
    #ylim = c(20, 30))
    theme_tq()

prices_base %>%
    tq_mutate(select = adjusted,mutate_fun = EMA, n = 30, col_rename = "ema_s") %>%
    tq_mutate(select = adjusted,mutate_fun = SMA, n = 200, col_rename = "sma_200") %>%
    plot_ly(x = ~date, y = ~adjusted, type = "scatter", mode = "lines", name = 'adjusted') %>%
    add_trace(y = ~ema_s, line = list(color = "green"), name = "ema 30") %>%
    add_trace(y = ~sma_200, line = list(color = "darkred"), name = "ema 200") %>%
    layout(xaxis = list(rangeslider = list(visible = T)))

prices_atr <- prices_base %>%
    tq_mutate(mutate_fun = ATR) %>%
    tq_mutate(select = adjusted, mutate_fun = WMA, n = 20, col_rename = "MA") %>%
    tq_mutate(mutate_fun = chaikinVolatility, col_rename = "chaikinVol")

ggplotly(prices_atr %>%
             select(date, close, atr, MA, chaikinVol) %>%
             pivot_longer(-date) %>%
             mutate(name = as.factor(name) %>% fct_relevel("close","MA")) %>%
             ggplot(aes(x = date, y = value, color = name))+
             geom_line(alpha = 0.8, linewidth = 1) +
             #geom_smooth(se = F)+
             #scale_color_viridis_d(option = "D")+
             scale_color_brewer(palette = "Dark2")+
             facet_wrap(~name, scales = "free_y", ncol = 1)+
             theme_bw()+
             theme(legend.position = "none"))

prices_atr %>% ggplot(aes(x = date,y = adjusted))+
    geom_line( color = "lightgreen")+
    geom_line(aes(y = trueHi), color = "orange")

# autocorrelation
prices_earnings_tbl %>% 
  filter(symbol == "NVDA") %>% 
    plot_acf_diagnostics(date, adjusted)

prices_base %>% 
    plot_acf_diagnostics(date, adjusted, .ccf_vars = volume, .show_ccf_vars_only = T)

prices_base %>% 
  filter(symbol == "NVDA") %>% 
    plot_seasonal_diagnostics(date, adjusted, .interactive = F)


# * explore momentum indicator ----
prices_features_dt[symbol == "ACN",.(date,Close_momentum_21_252_126, close)] %>% 
    plot_ly(x = ~date, y = ~Close_momentum_21_252_126, type = "scatter", mode = "lines") %>% 
    add_trace(y = ~close/10)

prices_features_dt[symbol == "ACN",
                   melt.data.table(.SD,
    id.vars = "date",
    measure.vars = c("Close_momentum_21_252_126", "close")
)] %>% 
    ggplot(aes(x = date, y = value, color = variable))+
    geom_line()+
    facet_wrap(~variable, scales = "free_y")

prices_features_dt[symbol == "ACN",.(date,Close_momentum_21_252_126, close, Return_fwd_21)] %>% 
    plot_ly(y = ~Return_fwd_21, x = ~Close_momentum_21_252_126, type = "scatter")
    
    

# * monthly returns plot ----
returns <- prices_features_dt %>%
    dplyr::group_by(symbol) %>%
    tq_transmute(select     = close,
                 mutate_fun = periodReturn,
                 period     = 'monthly',
                 col_rename = "close_ret") %>% 
    ungroup()

returns %>% 
    filter(symbol == "ACN") %>% 
    plot_ly(x = ~year(date), y = ~month(date), z = ~close_ret, type = "heatmap",
            colors = c("red","white","darkblue"),
            text = ~close_ret)

returns %>% 
    filter(symbol == "ACN") %>% 
    plot_seasonal_diagnostics(date, close_ret)

prices_features_dt[symbol == "ACN",] %>% 
    plot_seasonal_diagnostics(date, Return_fwd_21)

# Create full data ----
# * full data ----
full_data <- prices_earnings_econ_tbl %>% 
    
    filter(!is.na(Close_momentum_21_252_126)) %>% # filter NA on momentum since it is the longest indicator
    
    # fix data issues 
    # could move the prices_earnings to this space...
    # select(date, symbol, Return_fwd_21) %>% 
    # group_by(symbol) %>% 
    # pad_by_time(date, .by = 'day', .pad_value = 0) %>% 
    # ungroup() %>% 
    
    # remove unneeded features
    select(-(open:adjusted),
           #-Return_fwd_5, -Return_fwd_10,
           -num_ann,
           -contains("_lag_"),
           -contains("_lead_")) %>%
    relocate(date, 
             symbol, 
             starts_with("Close_"), 
             starts_with("Vol_"), 
             starts_with("Return")) %>% 
    
    # global features
    # mutate(Return_fwd_21 = log1p(Return_fwd_21)) %>% 
    
    # groupwise features
    group_by(symbol) %>% 
  mutate(est_eps_norm = standardize_vec(est_eps),
         act_eps_norm = standardize_vec(act_eps),
         eps_surprise_norm = standardize_vec(eps_surprise),
         eps_growth_norm = standardize_vec(eps_growth)) %>% 
  #relocate(est_eps_norm:act_eps_norm, .after = adjusted) %>% 
    # future_frame(date, .length_out = 28, .bind_data = T) %>% 
    # group_split() %>% 
    # map(.f, add_best_features(df, "adjusted")) %>% 
    # bind_rows() %>% 
  ungroup() %>% 
  select(-est_eps, -act_eps, -eps_surprise, -eps_growth) %>% 
  mutate(symbol = as_factor(symbol)) %>% 
  
  # lags / rolling / fourier features # fourier since it is panel data, different features by series
  group_by(symbol) %>% 
  arrange(symbol,date) %>% 
  
  # removing the fourier features since these do not really add to accuracy or shap values
  tk_augment_fourier(date, .periods = c(63,252), .K = 1) %>%
  # tk_augment_timeseries_signature(date) %>% 
  # select(-matches("(.xts$)|(.iso$)|(.hour)|(minute)|(second)|(am.pm)"), -(hour:hour12), -diff) %>% 
    # tk_augment_holiday_signature(date,.holiday_pattern = "US_",.locale_set = "US", .exchange_set = "NYSE") %>% 
    # tk_augment_lags(.value = Close_roc_0_1, .lags = c(5, 10)) %>% 
    # tk_augment_slidify(
    #     Return_fwd_21_lag28,
    #     .f       = ~ mean(.x, na.rm = T),
    #     .period  = c(7, 28, 28*2),
    #     .partial = TRUE,
    #     .align   = 'center'
    # ) %>% 
  ungroup() %>% 
    rowid_to_column(var = 'rowid') %>% 
  setDT()

setorderv(full_data,cols = c("date","symbol"))

# add interaction variables
# full_data[,":=" (eps_growth_eps_ann_days       = eps_growth * eps_ann_days,
#                      eps_surprise_eps_ann_days     = eps_surprise * eps_ann_days,
#                      value_FEDFUNDS_econ_ann_days  = value_FEDFUNDS * econ_ann_days,
#                      value_UNRATE_econ_ann_days    = value_UNRATE * econ_ann_days,
#                      change_CPILFESL_econ_ann_days = change_CPILFESL * econ_ann_days)]

# * full data correlation visualization ----
full_data %>% 
  select(starts_with("Close_"), Return_fwd_21, -contains("date")) %>% 
  drop_na() %>% 
    slice_sample(prop = 0.5) %>% 
  cor_test() %>% #filter(var1 == "Return_fwd_21" & var1 != var2) %>% arrange(desc(abs(cor)))
  plot_ly(x = ~var1,
          y = ~var2,
          z = ~cor,
          type = "heatmap",
          colors = c("darkred","white","darkgreen"))

full_data %>% 
    select(contains("gold"),contains("crude"), Return_fwd_21) %>% 
    drop_na() %>% 
    slice_sample(prop = 0.5) %>% 
    cor_test() %>% #filter(var1 == "Return_fwd_21" & var1 != var2) %>% arrange(desc(abs(cor)))
    plot_ly(x = ~var1,
            y = ~var2,
            z = ~cor,
            type = "heatmap",
            colors = c("darkred","white","darkgreen"))

full_data %>% 
  select(starts_with("date_"), starts_with("spy"), Return_fwd_21) %>% 
  drop_na() %>% 
  mutate(spy_state = as.numeric(spy_state)) %>% 
  cor_test() %>% #View()
  plot_ly(x = ~var1,
          y = ~var2,
          z = ~abs(cor),
          type = "heatmap",
          colors = c("white","darkgreen"))

full_data %>% 
  select(contains("div"), Return_fwd_21) %>% 
  drop_na() %>% 
  cor_test() %>% #View()
  plot_ly(x = ~var1,
          y = ~var2,
          z = ~abs(cor),
          type = "heatmap",
          colors = c("white","darkgreen"))

full_data %>% 
    select(contains("value"),contains("change"), Return_fwd_21) %>% 
    drop_na() %>% 
    cor_test() %>% #View()
    plot_ly(x = ~var1,
            y = ~var2,
            z = ~abs(cor),
            type = "heatmap",
            colors = c("white","darkgreen"))

# Save data for future use ----
write_rds(full_data, str_glue("01_save_data/{today()}_full_data_test.rds"))

# * remove unneeded data from environment ----
rm(prices_features_list)
rm(earnings_data)
rm(earnings_data_list)
rm(economic_data_cast)
rm(economic_data_pad)
rm(economic_data_final)
rm(prices_earnings_tbl)
rm(prices_earnings_econ_tbl)
rm(prices_features_dt)
rm(prices_div_dt)
rm(prices_base)
rm(dividends_tbl)
rm(economic_data)
# not rm(prices_dt) since we can use this for the optimization
gc()

#rm(list=ls(pattern="^wflw_"))

# 2.0 TIME TEST/TRAIN SPLIT ----

# * prepare for splitting ----
# removes the lines missing data due to the rolling periods
data_prepared_tbl <- full_data %>% 
  filter(!is.na(Return_fwd_21)) # %>% # test moving this filter to the full data to reduce the full data
    # filter(!is.na(Close_momentum_21_252_126))

setorderv(prices_features_dt, c("symbol","date"))

data_prepared_dt_short <- setDT(prices_features_dt[!is.na(Close_momentum_21_252_126) &
                                                       !is.na(Vol_WAP_norm),
                                               select(.SD,
                                                      -(open:adjusted),
                                                      #-Return_fwd_5, -Return_fwd_10,
                                                      -contains("_lag_"),
                                                      -contains("_lead_"))] %>% 
                                    tk_augment_fourier(date, .periods = c(252), .K = 1))

# prices_features_dt[,.(Return_fwd_21, shift(Return_fwd_21,1))] %>% cor_test()
# data_prepared_dt_short[,Return_fwd_21_shift_21 := shift(Return_fwd_21,21), keyby = symbol]

data_prepared_dt_short[,rowid := 1:.N]

setcolorder(data_prepared_dt_short, "rowid", before = "symbol")
    
forecast_tbl <- full_data %>% filter(is.na(Return_fwd_21))

forecast_dt <- data_prepared_dt_short[is.na(Return_fwd_21),]

data_prepared_dt_filter <- data_prepared_dt_short[!is.na(Return_fwd_21) & date >= train_date,]

# setnafill(data_prepared_tbl, type = "const", fill = 0, cols = "eps_surprise_norm")

# * split ----
# ** TS Split ----
splits <- data_prepared_tbl %>% 
    time_series_split(
    date_var = date,
    initial = 2.5*252,#"2.5 years",
    assess  = "30 days",
    cumulative = F
    )

# data_prepared_dt_short[!is.na(Return_fwd_21)
#& !is.na(Return_fwd_21_shift_21)]


splits <- data_prepared_dt_filter %>% 
    #select(Return_fwd_21, date, symbol, all_of(top_features)) %>% 
    time_series_split(
        date_var = date,
        initial = 700, #252*2.7,
        assess  = 21,
        cumulative = T
    )

set.seed(123)
train_short <- training(splits) %>% slice_sample(prop = 0.5, by = symbol)

# ** Normal split ----
split <- initial_split(data_prepared_dt_filter, prop = 0.8, strata = symbol)
train <- training(split)
test  <- testing(split)

# ** plot time plan ----
splits %>% 
    tk_time_series_cv_plan() %>% 
    plot_time_series_cv_plan(.date_var = date, .value = Return_fwd_21)

# * Clean Training Set ----
# - With Panel Data, need to do this outside of a recipe
# - Transformation happens by group

train_cleaned <- training(splits) %>% #summary()
  group_by(symbol) %>%
  #mutate(Return_fwd_21 = ts_clean_vec(Return_fwd_21, period = 7)) %>% 
  drop_na(Close_momentum_21_252_126) %>% 
  # select(-(date_sin5_K1:date_cos21_K2)) %>% 
  #slice_sample(prop = 0.7) %>% 
  ungroup() %>% 
  droplevels()

# ** plot splits ----
#train_cleaned %>% 
training(splits) %>% 
    group_by(symbol) %>% 
    plot_time_series(
        date, Return_fwd_21,
        .facet_vars = symbol,
        .facet_ncol = 3,
        .smooth = F,
        .trelliscope = T
    )

# 3.0 RECIPE ----

# * Recipe Specification ----

recipe_spec <- recipe(Return_fwd_21 ~ ., 
                      #data = training(splits)
                      data = train
                      ) %>%
    update_role(rowid, new_role = 'identifier') %>% 
    update_role(symbol, new_role = 'symbol') %>%
    # step_mutate_at(symbol, fn = droplevels) %>%
    # step_timeseries_signature(date) %>%
    # step_rm(matches("(.xts$)|(.iso$)|(.hour)|(.minute)|(.second)|(.am.pm)")) %>%
    # step_rm(date_index.num, date_year) %>%
    # step_normalize(date_index.num, ends_with("_year")) %>%  
    # step_normalize(index.num,year) %>% 
    step_rm(Return_fwd_5, Return_fwd_10) %>%
    # step_rm(number_dividends) %>% 
    # step_dummy(all_nominal_predictors(), one_hot = T, keep_original_cols = F) %>%
    # step_mutate(spy_state = as.factor(spy_state)) %>% 
    # step_interact(~Close_macd_long:Close_macd_short) %>% 
    step_interact(~Close_macd_long_signal:Close_macd_short_signal) %>% 
    #step_interact(~div_roc:div_ann_days) %>%
    step_interact(~eps_growth_norm:eps_ann_days) %>%
    step_interact(~eps_surprise_norm:eps_ann_days) %>%
    #step_interact(~value_FEDFUNDS:econ_ann_days) %>%
    #step_interact(~value_UNRATE:econ_ann_days) %>%
    #step_interact(~change_CPILFESL:econ_ann_days) %>%
    step_interact(~change_GDPC1:econ_ann_days) %>% 
    step_rm(econ_ann_days, change_GDPC1) %>% 
    step_filter_missing(all_predictors(), threshold = 0.2) %>% 
    step_zv(all_predictors()) %>% 
    step_nzv(all_predictors(), unique_cut = 0.02) #%>% 
    # step_corr(all_numeric_predictors(), threshold = 0.95) %>% 
    # step_pca(all_numeric_predictors())

# ** short data recipe spec ----
recipe_spec <- recipe(Return_fwd_21 ~ ., 
                      #data = training(splits)
                      data = train_short
                      ) %>% 
    update_role(rowid, new_role = 'identifier') %>% 
    update_role(symbol, new_role = 'symbol') %>%
    # step_mutate_at(symbol, fn = droplevels) %>%
    # step_timeseries_signature(date) %>%
    # step_rm(matches("(.xts$)|(.iso$)|(.hour)|(.minute)|(.second)|(.am.pm)")) %>%
    # step_rm(date_index.num, date_year) %>%
    # step_normalize(date_index.num, ends_with("_year")) %>%  
    # step_normalize(index.num,year) %>% 
    # step_rm(Return_fwd_5, Return_fwd_10) %>%
    # step_rm(Close_TRIX,Close_ADX_DX,EMV,rvi) %>% 
    # step_log(SAR) %>% 
    step_dummy(all_nominal_predictors(), one_hot = T, keep_original_cols = F) %>%
    # step_interact(Close_roc_long:Close_roc_short) %>% 
    step_interact(~Close_macd_long:Close_macd_short) %>% 
    step_interact(~Close_macd_long_signal:Close_macd_short_signal) %>% 
    step_filter_missing(all_predictors(), threshold = 0.2) %>% 
    step_zv(all_predictors())
    # step_nzv(all_predictors(), unique_cut = 0.02) #%>% 
# step_corr(all_numeric_predictors(), threshold = 0.95) %>% 
# step_pca(all_numeric_predictors())

# recipe_spec %>% prep() %>% bake(new_data = training(splits) %>% slice_sample(prop = 0.3)) %>% glimpse()
recipe_spec %>% prep() %>% juice() %>% glimpse()

# cols <- recipe_spec %>% prep() %>% juice() %>% colnames()
# cols_cor <- recipe_spec %>% step_corr(all_numeric_predictors(), threshold = 0.95) %>% prep() %>% juice() %>% colnames()
# 
# setdiff(cols, cols_cor)

# 4.0 HYPERPARAMETER TUNING MODELS ---- 

# * RESAMPLES - K-FOLD ----- 
set.seed(69)
# resamples_kfold <- training(splits) %>% vfold_cv(v = 5)
resamples_kfold_short <- train_short %>% vfold_cv(v = 5)

# * plot resampling folds ----
resamples_kfold %>% 
    tk_time_series_cv_plan() %>% 
    plot_time_series_cv_plan(date, Return_fwd_21, .facet_ncol = 2)

# Parallel Processing ----
# cl <- parallel::makeCluster(2, timeout = 60)
# plan(cluster, workers = cl)

options(future.globals.maxSize = 2.0 * 1024^3)  # Set to 2.0 GiB

parallel_start(1:4, .method = "future")

parallel_stop()

plan(sequential)

# * LightGBM TUNE ----

# ** Tunable Specification ----

model_spec_lgb_tune <- boost_tree(
  "regression",
  mtry           = tune(),
  trees          = tune(),
  min_n          = tune(),
  tree_depth     = tune(),
  learn_rate     = tune(),
  #loss_reduction = 0.01,#tune()
  stop_iter      = 20) %>% 
  set_engine('lightgbm', counts = F, validation = 0.1, num_threads = -1)

wflw_spec_lgb_tune <- workflow() %>% 
    add_model(model_spec_lgb_tune) %>% 
    # add_recipe(recipe_spec %>% step_rm(contains("63")) %>% step_rm(date))
    add_recipe(recipe_spec %>% step_rm(contains("date")) %>% step_rm(Vol_WAP_log)) #	
    # add_recipe(recipe_spec %>% step_rm(date))

# ** Tuning
set.seed(69)
start_l <- Sys.time()
tune_results_lgb <- wflw_spec_lgb_tune %>% 
  tune_race_anova(
    # resamples = resamples_kfold,
    resamples = resamples_kfold_short,
    param_info = extract_parameter_set_dials(wflw_spec_lgb_tune) %>% 
      update(learn_rate  = learn_rate(range = c(0.05, 0.5), trans = NULL)
             ,trees      = trees(range = c(200,3500))
             ,mtry       = mtry_prop(range = c(0.1,0.8))
      ),
    grid    = 10,
    control = control_race(verbose = T, parallel_over = NULL)
  )
end_l <- Sys.time()
end_l-start_l

# bayesian tuning shows slightly better results, takes however ~3x the time...
# set.seed(69)
# start_l <- Sys.time()
# tune_results_lgb <- wflw_spec_lgb_tune %>% 
#     tune_bayes(
#         # resamples = resamples_kfold,
#         resamples = resamples_kfold_short,
#         param_info = extract_parameter_set_dials(wflw_spec_lgb_tune) %>% 
#             update(learn_rate  = learn_rate(range = c(0.05, 0.5), trans = NULL)
#                    ,trees      = trees(range = c(200,3500))
#                    ,mtry       = mtry_prop(range = c(0.1,0.8))
#             ),
#         initial = 6,
#         control = control_bayes(verbose = T, parallel_over = NULL, seed = 101)
#     )
# end_l <- Sys.time()
# end_l-start_l


# ** Results with in-sample data

tune_results_lgb %>% collect_metrics()

tune_results_lgb %>% show_best(metric = "rmse", n = Inf)
tune_results_lgb %>% show_best(metric = "rsq",  n = Inf)

# ** Finalize

tic()
wflw_fit_lgb_tuned <- wflw_spec_lgb_tune %>% 
  finalize_workflow(select_best(tune_results_lgb, metric = "rmse")) %>% 
    #fit(train_short)
    fit(training(splits))
toc()

# ** test accuracy ----
augment(wflw_fit_lgb_tuned,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_lgb_tuned,testing(splits)) %>% #_short
    rsq(.pred, Return_fwd_21)

# calibrate_and_plot(wflw_fit_lgb_tuned, plot = F)

testing_symbol <- 'AAPL'

fcst_test_fit_lgb_tuned <- modeltime_table(wflw_fit_lgb_tuned) %>% 
    modeltime_calibrate(new_data = testing(splits),
                        id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21),], 
                       new_data = testing(splits),
                       keep_data  = T, # keeps the grouping variable and base data
                       conf_by_id = T)

lgboost_winrate <- fcst_test_fit_lgb_tuned %>% winrate("lightgbm")

lgboost_winrate[,.(total_wins = sum(win), winrate = mean(win))]

roc_auc(lgboost_winrate, actual_fact, pred_gain)
recall(lgboost_winrate, actual_fact, pred_fact)
conf_mat(lgboost_winrate, actual_fact, pred_fact)

lgboost_winrate[,.(total_wins = sum(win), winrate = mean(win)), keyby = symbol][winrate > mean(winrate),]

fcst_test_fit_lgb_tuned %>% 
    filter(symbol == "XEL") %>% 
  plot_modeltime_forecast(.conf_interval_show = F)

fcst_test_fit_lgb_tuned %>% 
    #filter(symbol == "ACN") %>% 
    group_by(symbol) %>% 
    plot_modeltime_forecast(.conf_interval_show = F,
                            .facet_ncol = 2,
                            #.facet_vars = symbol,
                            .trelliscope = T)

fcst_test_fit_lgb_tuned %>% plot_fcst()

# ** save tune results ----
write_rds(tune_results_lgb, "02_models/tune_results_lgb.rds")
rm(tune_results_lgb)
rm(wflw_spec_lgb_tune) # can recreate spec easily

gc()

# * XGBOOST TUNE ----

# ** Tunable Specification ----
model_spec_xgboost_tune <- boost_tree(
    "regression",
    mtry           = tune(),
    trees          = tune(),
    min_n          = tune(),
    tree_depth     = tune(),
    learn_rate     = tune(),
    # loss_reduction = 0.0005,#tune(),
    stop_iter      = 20
    ) %>% 
    set_engine('xgboost', 
               counts = F, 
               nthread =  -1, 
               tree_method = "hist",
               validation = 0.1)

wflw_spec_xgboost_tune <- workflow() %>% 
    add_model(model_spec_xgboost_tune) %>%
    # add_recipe(recipe_spec %>% step_rm(date))
    add_recipe(recipe_spec %>% step_rm(contains("date")))

# ** Tuning

set.seed(69)
start_x <- Sys.time()
tune_results_xgboost <- wflw_spec_xgboost_tune %>% 
    tune_race_anova(
        resamples = resamples_kfold,
        # resamples = resamples_kfold_short,
        param_info = extract_parameter_set_dials(wflw_spec_xgboost_tune) %>% 
          update(learn_rate = learn_rate(range = c(0.05, 0.5), trans = NULL),
                 trees      = trees(range = c(200,3500)),
                 mtry       = mtry_prop(range = c(0.1,0.9))
                 ),
        grid = 10,
        control = control_race(verbose = F, parallel_over = NULL)
    )
end_x <- Sys.time()
end_x-start_x

# ** Results

tune_results_xgboost %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_xgboost %>% 
    show_best(metric = "rsq", n = Inf)

collect_metrics(tune_results_xgboost)


# ** Finalize

start <- Sys.time()
wflw_fit_xgboost_tuned <- wflw_spec_xgboost_tune %>% 
    finalize_workflow(select_best(tune_results_xgboost, metric = "rmse")) %>% 
    fit(training(splits))
    # fit(train_short)
end <- Sys.time() 
end-start

# ** test accuracy ----
# calibrate_and_plot(wflw_fit_xgboost_tuned, plot = F)

augment(wflw_fit_xgboost_tuned,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_xgboost_tuned,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

fcst_test_fit_xgboost_tuned <- modeltime_table(wflw_fit_xgboost_tuned) %>% 
  modeltime_calibrate(new_data = testing(splits), id = "symbol") %>% 
  modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21),], 
                     new_data = testing(splits),
                        keep_data = T, conf_by_id = T) 

fcst_test_fit_xgboost_tuned <- augment(wflw_fit_xgboost_tuned, test)
fcst_test_fit_xgboost_tuned %>% rmse(.pred, Return_fwd_21)
fcst_test_fit_xgboost_tuned %>% rsq(.pred, Return_fwd_21)

fcst_test_fit_xgboost_tuned %>% 
    filter(symbol == "AAPL") %>% 
    select(date,.pred,Return_fwd_21) %>% pivot_longer(-date) %>%  ggplot(aes(x = date,y = value, color = name))+geom_line()

xgboost_winrate <- fcst_test_fit_xgboost_tuned %>%  winrate("xgboost")

xgboost_winrate[,.(total_wins = sum(win), winrate = mean(win), avg_gain = mean(actual_gain), pred_gain = mean(pred_gain))]

roc_auc(xgboost_winrate, actual_fact, pred_gain)
recall(xgboost_winrate, actual_fact, pred_fact)
conf_mat(xgboost_winrate, actual_fact, pred_fact)
f_meas(xgboost_winrate, actual_fact, pred_fact)

fcst_test_fit_xgboost_tuned %>% 
    filter(symbol == "AAPL") %>% 
    plot_modeltime_forecast(.conf_interval_show = F)

fcst_test_fit_xgboost_tuned %>% plot_fcst()

merge(lgboost_winrate, xgboost_winrate, by = "symbol")
#merge(xgboost_winrate, xgboost_winrate_long, by = "symbol")

# ** save tune results ----
write_rds(tune_results_xgboost, "02_models/tune_results_xgboost.rds", compress = "gz")
rm(tune_results_xgboost)
rm(wflw_spec_xgboost_tune) # remove spec to save memory

gc()

# * Prophet XGBoost TUNE ----

# ** Tunable Specification

model_spec_prophet_boost_tune <- prophet_boost(
    #changepoint_num    = 25,
    #changepoint_range  = 0.8,
    seasonality_yearly = F, #all seasonalities to F because xgboost does that
    seasonality_weekly = F,
    seasonality_daily  = F,
    mtry              = tune(),
    trees             = 500, # leave so it stabilizes the model #validate changing from 400 trees
    #min_n             = tune(),
    tree_depth        = tune(),
    learn_rate        = tune(),
    #loss_reduction    = 0.0005, #tune()
    stop_iter         = 20
) %>% set_engine("prophet_xgboost",  
                 counts = F, 
                 nthread =  -1, 
                 tree_method = "hist"
                 ,validation = 0.1)

wflw_spec_prophet_boost_tune <- workflow() %>% 
    add_model(model_spec_prophet_boost_tune) %>% 
    # add_recipe(recipe_spec_short)
    add_recipe(recipe_spec)

# ** Tuning

set.seed(69)
start_p <- Sys.time()
tune_results_prophet_boost <- wflw_spec_prophet_boost_tune %>% 
    tune_race_anova(
        resamples = resamples_kfold,
        # resamples = resamples_kfold_short,
        param_info = extract_parameter_set_dials(wflw_spec_prophet_boost_tune) %>% 
            update(learn_rate = learn_rate(range = c(0.1, 0.4), trans = NULL), # test changing to .05,0.5 to match xgboost
                   #trees      = trees(range = c(50,1000)),
                   mtry       = mtry_prop(range = c(0.1,0.8)) # test changing to .05,0.8 to match xgboost
            ),
        grid = 6,
        control = control_race(verbose_elim = T, parallel_over = NULL)
    )
end_p <- Sys.time() 
end_p-start_p

# ** Results
collect_metrics(tune_results_prophet_boost)

tune_results_prophet_boost %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_prophet_boost %>% 
    show_best(metric = "rsq", n = Inf)

# ** Finalize

start <- Sys.time()
wflw_fit_prophet_boost_tuned <- wflw_spec_prophet_boost_tune %>% 
    finalize_workflow(select_best(tune_results_prophet_boost, metric = "rmse")) %>% 
    fit(training(splits))
    # fit(training(splits_short))
end <- Sys.time()
end - start

# ** Prophet boost test accuracy ----
augment(wflw_fit_prophet_boost_tuned,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_prophet_boost_tuned,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

# calibrate_and_plot(wflw_fit_prophet_boost_tuned, plot = F)


fcst_test_fit_prophet_boost_tuned <- modeltime_table(wflw_fit_prophet_boost_tuned) %>% 
    modeltime_calibrate(new_data = testing(splits), id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21)], 
                       new_data = testing(splits),
                       keep_data = T, conf_by_id = T)

fcst_test_fit_prophet_boost_tuned <- augment(wflw_fit_prophet_boost_tuned, test)

fcst_test_fit_prophet_boost_tuned %>% 
    filter(symbol == "AAPL") %>% 
    select(date,.pred,Return_fwd_21) %>% pivot_longer(-date) %>%  ggplot(aes(x = date,y = value, color = name))+geom_line()
    plot_modeltime_forecast(.conf_interval_show = F)

# fcst_test_fit_prophet_boost_tuned %>% 
#     group_by(symbol) %>% 
#     plot_modeltime_forecast(.facet_vars = symbol, .facet_ncol = 4, .facet_scales = "fixed")

prophet_winrate <- fcst_test_fit_prophet_boost_tuned %>%
    winrate(model = "prophet_xgb")

prophet_winrate <- fcst_test_fit_prophet_boost_tuned %>% rename(prediction = .pred, 
                                             actual     = Return_fwd_21) %>% 
    mutate(pred_gain   = if_else(prediction > 0, 1, 0),
           actual_gain = if_else(actual > 0, 1, 0),
           win         = if_else(pred_gain == actual_gain, 1, 0),
           pred_fact   = as.factor(pred_gain),
           actual_fact = as.factor(actual_gain))

fcst_test_fit_prophet_boost_tuned %>% plot_fcst(scales = "free_y")

prophet_winrate[,.(total_wins = sum(win), total_rate = mean(win))]
roc_auc(prophet_winrate, actual_fact, pred_gain)
recall(prophet_winrate, actual_fact, pred_fact)
conf_mat(prophet_winrate, actual_fact, pred_fact)

# merge(prophet_winrate, prophet_winrate_long, by = "symbol")

# save tune results
write_rds(tune_results_prophet_boost, "02_models/tune_results_prophet_boost.rds")
rm(tune_results_prophet_boost)
rm(wflw_spec_prophet_boost_tune) # remove spec to save memory

gc()

# * Test EARTH TUNE ----
# tends to forecast a straight line.

# ** Tunable Specification

model_spec_earth_tune <- mars(
    "regression",
    num_terms   = tune(),
    prod_degree = tune()
    ) %>% 
  set_engine("earth")

wflw_spec_earth_tune <- workflow() %>% 
  add_model(model_spec_earth_tune) %>% 
  add_recipe(recipe_spec_short %>% step_rm(date))

# ** Tuning

start <- Sys.time()
set.seed(69)
tune_results_earth <- wflw_spec_earth_tune %>% 
    tune_grid(
        resamples = resamples_kfold_short,
        grid = 6,
        control = control_grid(verbose = T, parallel_over = NULL)
    )
end <- Sys.time() 
end-start

# ** Results
tune_results_earth %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_earth %>% 
    show_best(metric = "rsq", n = Inf)

# ** Finalize

wflw_fit_earth_tuned <- wflw_spec_earth_tune %>% 
    finalize_workflow(select_best(tune_results_earth, metric = "rmse")) %>% 
    fit(training(splits_short))

gc()

augment(wflw_fit_earth_tuned,testing(splits_short)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_earth_tuned,testing(splits_short)) %>% 
    rsq(.pred, Return_fwd_21)

fcst_test_fit_earth <- modeltime_table(wflw_fit_earth_tuned) %>% 
    modeltime_calibrate(new_data = testing(splits_short), id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short, 
                       new_data = testing(splits_short),
                       keep_data = T, conf_by_id = T) 

earth_winrate <- fcst_test_fit_earth %>%
    filter(!is.na(.model_id)) %>% 
    select(.index, symbol, .value, Return_fwd_21) %>% 
    rename(prediction = .value, actual = Return_fwd_21) %>% 
    #filter(.index == "2025-09-17") %>% 
    # pivot_wider(names_from = .key, values_from = .value) %>% 
    # filter(!is.na(prediction)) %>% 
    mutate(win = if_else((actual >= 0 & prediction >= 0)|
                             (actual < 0 & prediction < 0),
                         1, 0)) %>% 
    group_by(symbol) %>% 
    summarise(wins = sum(win), rate = mean(win), n = n()) %>% setDT()

earth_winrate[,.(total_wins = sum(wins), winrate = sum(wins)/sum(n))] 

fcst_test_fit_earth %>% 
    filter(symbol == "AAPL") %>% 
    plot_modeltime_forecast(.conf_interval_show = F)

# * Test CatBoost ----
# ** Catboost spec ----
# https://catboost.ai/docs/en/
# https://bonsai.tidymodels.org/
# https://bonsai.tidymodels.org/reference/train_catboost.html

model_spec_catboost <- boost_tree("regression",
                                  trees  = 1000
                                  #,tree_depth = 10
                                  #,learn_rate = 0.01
                                  #,mtry = 0.5
                                  ,stop_iter = 20
                                  ) %>% 
    set_engine('catboost'
               , eval_metric = "RMSE"
               , thread_count = 7
               # , early_stopping_rounds = 20
               # , od_type       = 'Iter'   # Early stopping
               # , od_wait       = 20
               # boosting_type = "Plain"
               # , counts = FALSE
               ) 



# set.seed(69)
# start <- Sys.time()
# wflw_fit_cb <- workflow() %>% 
#     add_model(model_spec_catboost) %>% 
#     add_recipe(recipe_spec %>% step_rm(date)) %>% 
#     fit(training(splits))
# # fit_resamples(resamples_kfold)
# end <- Sys.time()
# end-start


set.seed(69)
start <- Sys.time()
wflw_fit_cb_short <- workflow() %>% 
    add_model(model_spec_catboost) %>% 
    add_recipe(recipe_spec %>% step_rm(date)) %>% 
    #fit(train_short)
    fit_cv(resamples_kfold_short)
end <- Sys.time()
end-start

wflw_fit_cb
wflw_fit_cb_short
# collect_metrics(wflw_fit_cb)

extract_fit_engine(wflw_fit_cb_short) %>% 
    catboost::catboost.get_feature_importance(model = .) %>%
    as_tibble(rownames = "Feature") %>% 
    rename(Importance = V1) %>% 
    mutate(Feature = fct_reorder(Feature, Importance,.desc = F)) %>% View()

augment(wflw_fit_cb,training(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_cb,training(splits)) %>% 
    rsq(.pred, Return_fwd_21)

augment(wflw_fit_cb_short,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>%
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_cb_short,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

augment(wflw_fit_cb_short,testing(splits)) %>%
    rename(prediction = .pred, 
           actual     = Return_fwd_21) %>% 
    mutate(pred_gain   = if_else(prediction > 0, 1, 0),
           actual_gain = if_else(actual > 0, 1, 0),
           win         = if_else(pred_gain == actual_gain, 1, 0),
           pred_fact   = as.factor(pred_gain),
           actual_fact = as.factor(actual_gain)) |> 
    summarise(total_wins = sum(win), 
              winrate = mean(win), 
              gainers = mean(actual_gain), 
              lift = mean(win)/mean(actual_gain)) |> 
    mutate(model = "catboost_untuned")

# ** catboost tuning ----
# ** Tunable Specification

model_spec_catboost_tune <- boost_tree("regression",
                                       trees       = tune()
                                       ,tree_depth = tune()
                                       ,learn_rate = tune()
                                       # ,min_n = 20
                                       # ,mtry = 5
                                       , stop_iter = 20
                                       ) %>% 
    set_engine('catboost', thread_count = 2)  #counts = F, validation = 0.2

wflw_spec_catboost_tune <- workflow() %>% 
    add_model(model_spec_catboost_tune) %>% 
    # add_recipe(recipe_spec %>% step_rm(contains("date")))
    add_recipe(recipe_spec %>% step_rm(date))

# ** Tuning
set.seed(69)
start <- Sys.time()
tune_results_catboost <- wflw_spec_catboost_tune %>% 
    tune_race_anova(
        # resamples = resamples_kfold,
        resamples = resamples_kfold_short,
        # param_info = extract_parameter_set_dials(wflw_spec_catboost_tune) %>%
        #     update(trees = trees(range = c(100,2000))),
        grid = 8,
        control = control_race(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start


set.seed(69)
start <- Sys.time()
tune_results_catboost <- wflw_spec_catboost_tune %>% 
    tune_bayes(
        # resamples = resamples_kfold,
        resamples = resamples_kfold_short,
        # param_info = extract_parameter_set_dials(wflw_spec_catboost_tune) %>%
        #     update(trees = trees(range = c(100,2000))),
        # grid = 8,
        control = control_bayes(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start

# ** Results
collect_metrics(tune_results_catboost)

tune_results_catboost %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_catboost %>% 
    show_best(metric = "rsq", n = Inf)

# ** Finalize

wflw_fit_catboost_tuned <- wflw_spec_catboost_tune %>% 
    finalize_workflow(select_best(tune_results_catboost, metric = "rmse")) %>% 
    fit(training(splits))

# ** catboost accuracy on testing ----
augment(wflw_fit_catboost_tuned, testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>%
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_catboost_tuned, testing(splits)) %>% 
    metrics(.pred, Return_fwd_21)

calibrate_and_plot(wflw_fit_cb, plot = F)
calibrate_and_plot(wflw_fit_catboost_tuned, plot = F)

testing_symbol <- 'AAPL'

fcst_test_fit_catboost <- modeltime_table(wflw_fit_cb_short) %>%
    modeltime_calibrate(new_data = testing(splits), id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21)], 
                       new_data = testing(splits), keep_data = T)

catboost_winrate <- fcst_test_fit_catboost %>%
    winrate(model = 'catboost')

catboost_winrate[,.(total_wins = sum(win), winrate = mean(win))]

augment(wflw_fit_catboost_tuned, testing(splits)) %>% 
    rename(prediction = .pred, 
       actual     = Return_fwd_21) %>% 
    mutate(pred_gain   = if_else(prediction > 0, 1, 0),
           actual_gain = if_else(actual > 0, 1, 0),
           win         = if_else(pred_gain == actual_gain, 1, 0),
           pred_fact   = as.factor(pred_gain),
           actual_fact = as.factor(actual_gain)) |> 
    summarise(total_wins = sum(win), 
              winrate = mean(win), 
              gainers = mean(actual_gain), 
              lift = mean(win)/mean(actual_gain)) |> 
    mutate(model = "catboost_tuned")

roc_auc(catboost_winrate, actual_fact, pred_gain)
recall(catboost_winrate, actual_fact, pred_fact)
conf_mat(catboost_winrate, actual_fact, pred_fact)

# fcst_test_fit_catboost %>% plot_fcst()

fcst_test_fit_catboost %>% 
    filter(symbol == "AAPL") %>% 
    plot_modeltime_forecast(.conf_interval_show = F)

# merge(catboost_winrate, catboost_winrate_long, by = "symbol")
catboost_winrate[,.(total_wins = sum(wins), total_rate = sum(wins)/sum(n))]
merge(catboost_winrate, lgboost_winrate[,!c("n")], by = "symbol")
merge(catboost_winrate, xgboost_winrate[,!c("n")], by = "symbol")
# catboost_winrate_long %>% summarise(total_wins = sum(wins))

# save tune results
write_rds(tune_results_catboost, "02_models/tune_results_catboost.rds")
rm(tune_results_catboost)
rm(wflw_spec_catboost_tune) # can recreate spec easily

gc()

# * Test glmnet TUNE ----

# ** Tunable Specification

model_spec_glmnet_tune <- linear_reg(
  mode = "regression",
  penalty = tune(), # higher penalty removes features faster
  mixture = tune()) %>% 
  set_engine('glmnet')

wflw_spec_glmnet_tune <- workflow() %>% 
    add_model(model_spec_glmnet_tune) %>% 
    add_recipe(recipe_spec_short %>% step_rm(date))# %>% step_ts_impute(all_numeric_predictors()))

# ** Tuning
set.seed(69)
start <- Sys.time()
tune_results_glmnet <- wflw_spec_glmnet_tune %>% 
  tune_race_anova(
    # resamples = resamples_kfold,
    resamples = resamples_kfold_short,
    grid = 10,
    control = control_race(verbose = T, parallel_over = NULL)
  )
end <- Sys.time()
end-start

# ** Results
collect_metrics(tune_results_glmnet)

tune_results_glmnet %>% 
  show_best(metric = "rmse", n = Inf)

tune_results_glmnet %>% 
    show_best(metric = "rsq", n = Inf)

# ** Finalize

wflw_fit_glmnet_tuned <- wflw_spec_glmnet_tune %>% 
  finalize_workflow(select_best(tune_results_glmnet, metric = "rmse")) %>%
    # fit(training(splits_short))
    fit(training(splits))

# ** GLMNet testing accuracy ----

augment(wflw_fit_glmnet_tuned,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_glmnet_tuned,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

# calibrate_and_plot(wflw_fit_glmnet_tuned, plot =F)

fcst_test_fit_glmnet_tuned <- modeltime_table(wflw_fit_glmnet_tuned) %>% 
    modeltime_calibrate(new_data = testing(splits), id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21),], 
                       new_data = testing(splits),
                       keep_data = T, conf_by_id = T) 

fcst_test_fit_glmnet_tuned %>% 
    filter(symbol == "AAPL") %>% 
    plot_modeltime_forecast(.conf_interval_show = F)

glmnet_winrate <- winrate(fcst_test_fit_glmnet_tuned, "glmnet")

merge(glmnet_winrate, lgboost_winrate, by = c("symbol","date"))

glmnet_winrate[,.(total_wins = sum(win), winrate = mean(win))]
roc_auc(glmnet_winrate, actual_fact, pred_gain)
recall(glmnet_winrate, actual_fact, pred_fact)
conf_mat(glmnet_winrate, actual_fact, pred_fact)

# save tune results
write_rds(tune_results_glmnet, "02_models/tune_results_glmnet.rds")
rm(tune_results_glmnet)
rm(wflw_spec_glmnet_tune) # remove spec to save memory

gc()

# * Test RANDOM FOREST ranger ----
# - Strengths: Can model seasonality very well
# - Weaknesses: 
#   - Cannot predict beyond the maximum/minimum target (e.g. increasing trend)
# - Solution: Model trend separately (if needed). 
#   - Can combine with ARIMA, Linear Regression, Mars, or Prophet


# Model specification
model_spec_rf <- rand_forest(
    "regression",
    # trees = 1000, min_n = 1 #mtry = 25, min_n = 25
    ) %>% 
    set_engine("ranger", importance = 'permutation')#, splitrule  = "extratrees") 

# workflow
wflw_spec_rf <- workflow() %>% 
    add_model(model_spec_rf) %>% 
    add_recipe(recipe_spec %>% step_rm(date))

# Fit
set.seed(69)
start_r <- Sys.time()
wflw_fit_rf_cv <- wflw_spec_rf %>% 
    #fit(training(splits))
    # fit_resamples(resamples_kfold)
    fit_resamples(resamples_kfold_short)
end_r <- Sys.time()
end_r-start_r

# finalize RF
wflw_fit_rf_final <- wflw_spec_rf %>% 
    finalize_workflow(select_best(wflw_fit_rf_cv, metric = "rmse")) %>%
    # fit(training(splits_short))
    fit(training(splits))

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_rf,
    type = 'testing',
    plot = FALSE
)

# ** test accuracy -----
augment(wflw_fit_rf_final,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_rf_final,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

wflw_fit_rf_final %>% 
    extract_fit_engine() %>% 
    ranger::importance() %>% 
    enframe() %>% 
    mutate(rel_importance = value/sum(value)) %>% 
    slice_max(order_by = value, n = 20) %>% 
    mutate(Feature = as.factor(name) %>% fct_reorder(.x = rel_importance)) %>%
    plot_ly(x = ~rel_importance,
            y = ~Feature,
            alpha = 0.7,
            color = ~Feature,
            type = 'bar',
            colors = 'Dark2') %>% 
    layout(title = "Random Forest Feature Importance",
           showlegend = F,
           yaxis = list(title = ""))


fcst_test_fit_rf <- modeltime_table(wflw_fit_rf_final) %>%
    modeltime_calibrate(new_data = testing(splits), id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21)], 
                       new_data = testing(splits), keep_data = T)

ranger_winrate <- fcst_test_fit_rf %>%
    filter(!is.na(.model_id)) %>% 
    # filter(date == max(date)) %>% 
    select(.index, symbol, .value, Return_fwd_21) %>% 
    rename(prediction = .value, actual = Return_fwd_21) %>% 
    #filter(.index == "2025-09-17") %>% 
    # pivot_wider(names_from = .key, values_from = .value) %>% 
    # filter(!is.na(prediction)) %>% 
    mutate(win = if_else((actual >= 0 & prediction >= 0)|
                             (actual < 0 & prediction < 0),
                         1, 0)) %>% 
    group_by(symbol) %>% 
    summarise(wins = sum(win), rate = mean(win), n = n()) %>% 
    ungroup() %>% 
    mutate(model = 'randomforest') %>% setDT()

ranger_winrate[,.(total_wins = sum(wins), total_rate = sum(wins)/sum(n))]
merge(ranger_winrate, xgboost_winrate, by = "symbol")

# fcst_test_fit_catboost %>% plot_fcst()

fcst_test_fit_rf %>% 
    filter(symbol == "AAPL") %>% 
    plot_modeltime_forecast(.conf_interval_show = F)

# merge(catboost_winrate, catboost_winrate_long, by = "symbol")
merge(ranger_winrate, lgboost_winrate, by = "symbol")
merge(ranger_winrate, xgboost_winrate, by = "symbol")

# save tune results
write_rds(wflw_fit_rf, "02_models/wflw_fit_rf.rds")
rm(wflw_fit_rf)
#rm(wflw_spec_rf_tune) # no spec for rf tuning

gc()

# * Test Prophet lgb/cat Boost TUNE ----

# ** Tunable Specification

model_spec_prophet_boost_tune <- boost_prophet( #prophet_boost for modeltime
  #changepoint_num    = 25,
  #changepoint_range  = 0.8,
  seasonality_yearly = F, #all seasonalities to F because xgboost does that
  seasonality_weekly = F,
  seasonality_daily  = F,
  #mtry              = tune(),
  trees             = 100, # leave so it stabilizes the model
  #min_n             = tune(),
  tree_depth        = tune(),
  learn_rate        = tune()
  #loss_reduction    = tune()
) %>% set_engine("prophet_lightgbm", counts = F)

wflw_spec_prophet_boost_tune <- workflow() %>% 
  add_model(model_spec_prophet_boost_tune) %>% 
  add_recipe(recipe_spec)

# ** Tuning

set.seed(69)
start <- Sys.time()
tune_results_prophet_boost <- wflw_spec_prophet_boost_tune %>% 
  tune_grid(
    resamples = resamples_kfold, #training(splits) %>% vfold_cv(v = 2),
    param_info = extract_parameter_set_dials(wflw_spec_prophet_boost_tune) %>% 
      update(learn_rate = learn_rate(range = c(0.1, 0.4), trans = NULL)
             #trees      = trees(range = c(50,1000)),
             #mtry       = mtry_prop(range = c(0.1,0.9)
             ),
    grid = 2,
    control = control_grid(verbose = T, allow_par = F)
  )
end <- Sys.time() 
end-start

# ** Results

tune_results_prophet_boost %>% 
  show_best(metric = "rmse", n = Inf)

tune_results_prophet_boost %>% 
    show_best(metric = "rsq", n = Inf)

# ** Finalize

wflw_fit_prophet_boost_tuned <- wflw_spec_prophet_boost_tune %>% 
  finalize_workflow(select_best(tune_results_prophet_boost, metric = "rmse")) %>% 
  fit(training(splits))


# * Test dbarts ----
# Model specification

model_spec_dbarts <- bart(
    "regression"
) %>% 
    set_engine("dbarts") 

# Fit
set.seed(69)
start <- Sys.time()
wflw_fit_dbarts <- workflow() %>% 
    add_model(model_spec_dbarts) %>% 
    add_recipe(recipe_spec %>% step_rm(date)) %>% 
    fit_resamples(resamples_kfold)
end <- Sys.time()
end-start

collect_metrics(wflw_fit_dbarts)

wflw_fit_dbarts_final <- model_spec_dbarts %>% 
    finalize_workflow(select_best(wflw_fit_dbarts, metric = "rmse")) %>% 
    fit(training(splits))

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_dbarts_final,
    type = 'testing',
    plot = FALSE
)

modeltime_table(wflw_fit_dbarts_final) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)
    ) %>% 
    plot_modeltime_forecast()

# save tune results
write_rds(wflw_fit_dbarts, "02_models/wflw_fit_dbarts.rds")
rm(wflw_fit_dbarts)
#rm(wflw_spec_dbarts_tune) # no spec for dbarts tuning

gc()

# * Test h2o auto ml ----
# based off of https://www.r-bloggers.com/2025/11/taming-volatility-high-performance-forecasting-of-the-stoxx-600-with-h2o-automl/
# install.packages("h2o", type="source", repos="https://h2o-release.s3.amazonaws.com/h2o/rel-3.46.0/10/R")

# library(modeltime.h2o)

#Initialize H2O
h2o.init(
    nthreads = -1,
    ip       = 'localhost',
    port     = 54321
)

#Model specification and fitting
h2o_model_spec <- automl_reg(mode = 'regression') %>%
    set_engine(
        engine                     = 'h2o',
        #max_runtime_secs           = 120, 
        max_runtime_secs_per_model = 90,
        max_models                 = 15,
        nfolds                     = 6,
        exclude_algos              = c("DeepLearning"),
        #verbosity                  = NULL,
        seed                       = 98765
    ) 

# Fit
set.seed(69)
start <- Sys.time()
model_fit_h2o <- workflow() %>% 
    add_model(h2o_model_spec) %>% 
    add_recipe(recipe_spec_short) %>% 
    fit(train_short)
end <- Sys.time()
end-start

#Evaluation
model_fit_h2o %>% 
    automl_leaderboard()

# Calibrate & Plot
modeltime_table(model_fit_h2o) %>% 
    modeltime_accuracy(new_data = testing(splits), id = "symbol")

modeltime_table(model_fit_h2o) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol),
                       keep_data = T
    ) %>% 
    plot_modeltime_forecast()

# validation and performance
valid_h2o <- as.h2o(testing(splits) %>% bake(prep(recipe_spec), .))
model_h2o <- h2o.getModel("XRT_1_AutoML_3_20251103_102636")

perf.h2o <- h2o.performance(model_h2o, newdata = valid_h2o)

h2o.varimp(model_h2o)
h2o.varimp_plot(model_h2o)

shap_plot <- h2o.shap_summary_plot(model_h2o, valid_h2o, sample_size = 1000)

ggplotly(shap_plot)

# save model
model_h2o %>% 
    save_h2o_model(path = "02_models/", overwrite = TRUE)
model_h2o <- load_h2o_model(path = "02_models/")


h2o.shutdown(prompt = FALSE)


# timing by model
lb <- model_fit_h2o %>% 
    automl_leaderboard()
model_ids <- as.vector(lb$model_id)

durations <- sapply(model_ids, function(mid) {
    m <- h2o.getModel(mid)
    st <- as.POSIXct(m@model$start_time / 1000, origin = "1970-01-01", tz = "UTC")
    et <- as.POSIXct(m@model$end_time / 1000, origin = "1970-01-01", tz = "UTC")
    as.numeric(difftime(et, st, units = "secs"))
    #rt <- m@model$run_time
})

duration_df <- tibble(model_id = model_ids, duration_secs = durations)
duration_df
sum(duration_df$duration_secs)

# * Test GAM ----

# Model specification
library(mgcv)
model_spec_gam <- gen_additive_mod() |>
    set_engine("mgcv") |>
    set_mode("regression") 

gam_data <- training(splits) %>% 
  filter(symbol %in% c("AAPL","NVDA","AMD"))

model_fit_gam <- workflow() |>
  add_model(model_spec_gam, 
            formula = Return_fwd_21 ~ Close_macd_long + Close_macd_short + 
              Close_macd_long_signal + Close_macd_short_signal + 
              Vol_WAP_norm + date_cos252_K1 + 
              s(Vol_WAP_norm, k = 2) + s(Close_natr_63, k = 1) + s(Close_macd_long_signal, k = 1)) |>
  add_formula(Return_fwd_21 ~ Close_macd_long + Close_macd_short + 
                Close_macd_long_signal + Close_macd_short_signal + Vol_WAP_norm + Close_natr_63 + date_cos252_K1) |>
  fit(data = gam_data)

extract_fit_engine(model_fit_gam) %>% summary()


# Fit
set.seed(69)
start <- Sys.time()
wflw_fit_gam <- workflow() %>% 
    add_model(model_spec_gam) %>% 
    add_recipe(recipe_spec %>% step_rm(date)) %>% 
    fit(gam_data)
end <- Sys.time()
end-start

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_gam,
    type = 'testing',
    plot = FALSE
)

modeltime_table(wflw_fit_gam) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)
    ) %>% 
    plot_modeltime_forecast()

wflw_fit_gam %>% 
    extract_fit_engine() %>% 
    ranger::importance() %>% 
    enframe() %>% 
    mutate(rel_importance = value/sum(value)) %>% 
    slice_max(order_by = value, n = 20) %>% 
    mutate(Feature = as.factor(name) %>% fct_reorder(.x = rel_importance)) %>%
    plot_ly(x = ~rel_importance,
            y = ~Feature,
            alpha = 0.7,
            color = ~Feature,
            type = 'bar',
            colors = 'Dark2') %>% 
    layout(title = "Random Forest Feature Importance",
           showlegend = F,
           yaxis = list(title = ""))

# save tune results
write_rds(wflw_fit_rf, "02_models/wflw_fit_rf.rds")
rm(wflw_fit_rf)
#rm(wflw_spec_rf_tune) # no spec for rf tuning

gc()


# * Test NEURAL NET brulee ----
# ** Tuning
model_spec_nnet <- mlp(
    hidden_units = 20, #42 showed better RMSE and RSQ on tuning but 30 looks better from plots?
    #hidden_units = c(20,20), #try with multiple layers
    epochs       = 500
    
) %>%
    # set_engine("nnet")
    set_engine("brulee", 
               validation = 0.1, # 10% holdout for early stopping
               stop_iter  = 10
               ) %>% 
    set_mode("regression")

wflw_spec_nnet <- workflow() %>%
    add_model(model_spec_nnet) %>%
    add_recipe(recipe_spec %>% 
                   step_rm(date) %>% 
                   step_normalize(all_numeric_predictors()))

set.seed(101)
start <- Sys.time()
fit_nnet <- wflw_spec_nnet %>% fit(train_short)
end <- Sys.time()
end-start

augment(fit_nnet, testing(splits)) %>% 
    rmse(.pred, Return_fwd_21)

augment(fit_nnet, testing(splits)) %>% 
    metrics(.pred, Return_fwd_21)

set.seed(69)
start <- Sys.time()
fit_cv_nnet <- wflw_spec_nnet %>% 
    #fit(train_short)
    fit_resamples(resamples_kfold_short)
end <- Sys.time()
end-start

augment(wflw_fit_nnet, testing(splits)) %>% 
    rmse(.pred, Return_fwd_21)

augment(wflw_fit_nnet, testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

# augment(wflw_fit_nnet, testing(splits)) %>% plot_ly(x = ~.pred, y = ~Return_fwd_21, type = "scatter")

collect_metrics(wflw_fit_nnet)

wflw_fit_nnet_tuned <- wflw_spec_nnet %>% 
    finalize_workflow(select_best(fit_cv_nnet, metric = "rmse")) %>% 
    fit(training(splits))
                                        
# ** Tuning
model_spec_nnet_tune <- mlp(
    hidden_units = tune()
    #penalty      = tune() 
    ,epochs      = 500
    #,activation  = tune()
    #,learn_rate   = tune()
) %>%
    # set_engine("nnet")
    set_engine("brulee", 
               validation = 0.1, # 10% holdout for early stopping
               stop_iter  = 10, 
               rate_schedule = "cyclic"
               ) %>% 
    set_mode("regression")

wflw_spec_nnet_tune <- workflow() %>%
    add_model(model_spec_nnet_tune) %>%
    add_recipe(recipe_spec_short %>% 
                   step_rm(date) %>% 
                   step_normalize(all_numeric_predictors()))


grid_nnet <- grid_space_filling(hidden_units(range = c(20, 60)),
                                size = 5)

# cores <- parallelly::availableCores(omit = 1)
# library(future.mirai)
# plan(mirai_multisession, workers = cores)

set.seed(69)
start <- Sys.time()
tune_results_nnet <- tune_grid(
    wflw_spec_nnet_tune,
    resamples = resamples_kfold_short,
    grid      = grid_nnet,
    metrics   = metric_set(rmse, rsq),
    control   = control_grid(verbose = T, parallel_over = NULL)
)
end <- Sys.time()
end-start

set.seed(69)
start <- Sys.time()
tune_results_nnet <- wflw_spec_nnet_tune %>% 
    tune_race_anova(
        resamples  = resamples_kfold_short,
        # param_info = extract_parameter_set_dials(wflw_spec_nnet_tune) %>% 
        #     update(
        #         #epochs       = epochs(range = c(100,500)),
        #         hidden_units = hidden_units(range = c(20, 50))
        #     ),
        grid       = grid_nnet,
        control    = control_race(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start

set.seed(69)
start <- Sys.time()
tune_results_nnet <- wflw_spec_nnet_tune %>% 
    tune_bayes(
        resamples = resamples_kfold_short,
        initial = 8, # start with 10 random points
        iter    = 33, # then 30 Bayesian iterations
        metrics = metric_set(rmse, rsq),
        control = control_bayes(verbose = TRUE, parallel_over = NULL)
    )
end <- Sys.time()
end-start

plan(sequential)

# ** Results
collect_metrics(tune_results_nnet)

tune_results_nnet %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_nnet %>% 
    show_best(metric = "rsq", n = Inf)

autoplot(tune_results_nnet)

# ** Finalize

wflw_fit_nnet_tuned <- wflw_spec_nnet_tune %>% 
    finalize_workflow(select_best(tune_results_nnet, metric = "rmse")) %>% 
    fit(training(splits))

# accuracy on testing
augment(wflw_fit_nnet_tuned,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_nnet_tuned,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

augment(wflw_fit_nnet_tuned, testing(splits)) %>% 
    plot_ly(x = ~.pred, y = ~Return_fwd_21)

calibrate_and_plot(wflw_fit_nnet_tuned, plot = F)

# forecast and test 
#fcst_test_fit_nnet_tune <- modeltime_table(wflw_fit_nnet_tuned) %>%
fcst_test_fit_nnet <- modeltime_table(fit_nnet) %>% 
    modeltime_calibrate(new_data = testing(splits),
                        id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_dt_short[!is.na(Return_fwd_21),], 
                       new_data = testing(splits),
                       keep_data  = T, # keeps the grouping variable and base data
                       conf_by_id = T)

nnet_winrate <- fcst_test_fit_nnet %>%
    filter(.key == "prediction") %>% 
    # filter(date == max(date)) %>% 
    select(.index, symbol, .value, Return_fwd_21) %>% 
    rename(prediction = .value, actual = Return_fwd_21) %>% 
    #filter(.index == "2025-09-17") %>% 
    # pivot_wider(names_from = .key, values_from = .value) %>% 
    # filter(!is.na(prediction)) %>% 
    mutate(win = if_else((actual >= 0 & prediction >= 0)|
                             (actual < 0 & prediction < 0),
                         1, 0)) %>% 
    group_by(symbol) %>% 
    summarise(wins = sum(win), rate = mean(win), n = n()) %>% 
    ungroup() %>% 
    mutate(model = 'nnet') %>% setDT() 

# nnet_winrate_long <- copy(nnet_winrate)

nnet_winrate[,.(total_wins = sum(wins), total_rate = sum(wins)/sum(n))]

#fcst_test_fit_nnet_tune %>% 
fcst_test_fit_nnet %>%
    filter(symbol == "YUM") %>% 
    plot_modeltime_forecast(.conf_interval_show = T, .facet_vars = symbol)

write_rds(tune_results_nnet, "02_models/tune_results_nnet.rds", compress = "gz")
rm(tune_results_nnet)
rm(wflw_spec_nnet_tune) # remove spec to save memory

gc()

# * Test deep learning ----
library(tabnet)

# ** Tuning
model_spec_nnet_tune <- tabnet(
    mode = "regression",
    hidden_units = tune(),
    penalty = tune(), 
    epochs = tune()
) %>%
    set_engine("torch")

wflw_spec_nnet_tune <- workflow() %>%
    add_model(model_spec_nnet_tune) %>%
    add_recipe(recipe_spec_short %>% 
                   step_rm(date) %>% 
                   step_normalize(all_numeric_predictors()))


set.seed(69)
start <- Sys.time()
tune_results_nnet <- wflw_spec_nnet_tune %>% 
    tune_race_anova(
        resamples = resamples_kfold_short,
        grid = 4,
        control = control_race(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start

# ** Results

tune_results_nnet %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_nnet %>% 
    show_best(metric = "rsq", n = Inf)

# ** Finalize

wflw_fit_nnet_tuned <- wflw_spec_nnet_tune %>% 
    finalize_workflow(select_best(tune_results_nnet, metric = "rmse")) %>% 
    fit(training(splits))

# accuracy on testing
augment(wflw_fit_nnet_tuned,testing(splits)) %>% 
    mutate(error = .pred-Return_fwd_21) %>% 
    summarise(rmse = sqrt(mean(error^2)))

augment(wflw_fit_nnet_tuned,testing(splits)) %>% 
    rsq(.pred, Return_fwd_21)

calibrate_and_plot(wflw_fit_nnet_tuned, plot = F)

fcst_test_fit_nnet_tune <- modeltime_table(wflw_fit_nnet_tuned) %>% 
    modeltime_calibrate(new_data = testing(splits), id = "symbol") %>% 
    modeltime_forecast(actual_data = data_prepared_tbl, 
                       new_data = testing(splits), keep_data = T)

nnet_winrate <- fcst_test_fit_nnet_tune %>%
    select(-.model_id,-.model_desc, -.conf_lo, -.conf_hi) %>% 
    #filter(.index == "2025-09-17") %>% 
    pivot_wider(names_from = .key, values_from = .value) %>% 
    filter(!is.na(prediction)) %>% 
    mutate(win = if_else((actual >= 0 & prediction >= 0)|
                             (actual < 0 & prediction < 0),
                         1, 0)) %>% 
    summarise(count = n(),wins = sum(win), rate = mean(win)) %>% 
    setDT()

# nnet_winrate_long <- copy(nnet_winrate)

nnet_winrate[,.(total_wins = sum(wins))]

fcst_test_fit_nnet_tune %>% 
    filter(symbol == "ACN") %>% 
    plot_modeltime_forecast(.conf_interval_show = T, .facet_vars = symbol)

# * Test Cubist ----
# Model specification
library(rules)

model_spec_cubist_tune <- cubist_rules(
   committees = tune(), # 100
   neighbors = tune(), # 6
   max_rules = tune() # 333
) |>
    set_engine("Cubist")

wflw_spec_cubist_tune <- workflow() %>% 
    add_model(model_spec_cubist_tune) %>% 
    add_recipe(recipe_spec_short %>% step_rm(date))

# Fit
set.seed(69)
start <- Sys.time()
tune_results_cubist <- wflw_spec_cubist_tune %>% 
    tune_race_anova(
        resamples = resamples_kfold_short,
        grid = 7,
        control = control_race(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start

cubist_metrics <- collect_metrics(tune_results_cubist)

# metrics:
#     rmse 0.0234
#     rsq 0.954
# calibration metrics:
#     # A tibble: 1 × 9
#     .model_id .model_desc .type    mae  mape  mase smape  rmse   rsq
#     1         1 CUBIST      Test  0.0765  386. 0.784  122. 0.112 0.123
# .model_id .model_desc     .type    mae  mape  mase smape  rmse   rsq
#     1         1 CUBIST      Test  0.0715  404. 0.732  123. 0.105 0.141

wflw_fit_cubist_final <- wflw_spec_cubist_tune %>% 
    #finalize_workflow(select_best(wflw_fit_cubist, metric = "rmse")) %>% 
    finalize_workflow(select_best(tune_results_cubist, metric = "rmse")) %>%
    fit(training(splits))

# test forecast

fcst_test_fit_cubist_tuned <- modeltime_table(wflw_fit_cubist_final) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)
    ) 

fcst_test_fit_cubist_tuned %>% 
    plot_modeltime_forecast()

fcst_test_fit_cubist_tuned %>%
    select(-.model_id,-.model_desc, -.conf_lo, -.conf_hi) %>% 
    #filter(.index == "2025-09-17") %>% 
    pivot_wider(names_from = .key, values_from = .value) %>% 
    filter(!is.na(prediction)) %>% 
    mutate(win = if_else((actual >= 0 & prediction >= 0)|
                                   (actual < 0 & prediction < 0),
                               1, 0)) %>% 
    summarise(wins = sum(win), rate = mean(win))

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_cubist_final,
    type = 'testing',
    plot = FALSE
)

# save tune results
write_rds(tune_results_cubist, "02_models/tune_results_cubist.rds")
rm(tune_results_cubist)
#rm(wflw_spec_cubist_tune) # no spec for cubist tuning

gc()

# * Test XRF ----
# https://github.com/holub008/xrf

library(rules)

# Model specification
model_spec_xrf <- rule_fit(
    "regression"
     ,trees      = tune(), 
     tree_depth = tune(),
     learn_rate = tune(),
     penalty    = tune(),
    stop_iter = 20
) %>% 
    set_engine("xrf", validation = 0.2, deoverlap = TRUE) 

# Fit
set.seed(69)
start <- Sys.time()
wflw_fit_xrf <- workflow() %>% 
    add_model(model_spec_xrf) %>% 
    add_recipe(recipe_spec %>% step_rm(date)) %>% 
    fit(training(splits))
end <- Sys.time()
end-start

# tuning
wflw_spec_xrf_tune <- workflow() %>% 
    add_model(model_spec_xrf) %>% 
    add_recipe(recipe_spec %>% step_rm(date))

# Fit tuning
set.seed(69)
start <- Sys.time()
tune_results_xrf <- wflw_spec_xrf_tune %>% 
    tune_race_anova(
        resamples = resamples_kfold,
        grid = 4,
        control = control_race(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start

# ** Results

tune_results_xrf %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_xrf %>% 
    show_best(metric = "rsq", n = Inf)


# ** Finalize

start <- Sys.time()
wflw_fit_xrf <- wflw_spec_xrf_tune %>% 
    finalize_workflow(select_best(tune_results_xrf, metric = "rmse")) %>% 
    fit(training(splits))
end <- Sys.time() 
end-start

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_xrf,
    type = 'testing',
    plot = FALSE
)

modeltime_table(wflw_fit_xrf) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)
    ) %>% 
    plot_modeltime_forecast()


# save tune results
write_rds(wflw_fit_xrf, "02_models/wflw_fit_xrf.rds")
rm(wflw_fit_xrf)
#rm(wflw_spec_xrf_tune) # no spec for rf tuning

gc()

# * Test RANDOM FOREST partykit ----

# Model specification
model_spec_rf <- rand_forest(
    "regression"
    #trees = 1000, min_n = 1 #mtry = 25, min_n = 25
) %>% 
    set_engine("partykit") 

# Fit
registerDoParallel(cores = 4)
set.seed(69)
start <- Sys.time()
wflw_fit_rf <- workflow() %>% 
    add_model(model_spec_rf) %>% 
    add_recipe(recipe_spec %>% step_rm(date)) %>% 
    fit(training(splits) %>% slice_sample(prop = 0.1))
end <- Sys.time()
end-start
plan(sequential)

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_rf,
    type = 'testing',
    plot = FALSE
)

modeltime_table(wflw_fit_rf) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)
    ) %>% 
    plot_modeltime_forecast()

wflw_fit_rf %>% 
    extract_fit_engine() %>% 
    ranger::importance() %>% 
    enframe() %>% 
    mutate(rel_importance = value/sum(value)) %>% 
    slice_max(order_by = value, n = 20) %>% 
    mutate(Feature = as.factor(name) %>% fct_reorder(.x = rel_importance)) %>%
    plot_ly(x = ~rel_importance,
            y = ~Feature,
            alpha = 0.7,
            color = ~Feature,
            type = 'bar',
            colors = 'Dark2') %>% 
    layout(title = "Random Forest Feature Importance",
           showlegend = F,
           yaxis = list(title = ""))

# save tune results
write_rds(wflw_fit_rf, "02_models/wflw_fit_rf.rds")
rm(wflw_fit_rf)
#rm(wflw_spec_rf_tune) # no spec for rf tuning

gc()

# * Test bagged tree ----

# Model specification
model_spec_tree <- bag_tree(#tree_depth = integer(1), 
                          #min_n = integer(1), 
                          #cost_complexity = double(1)
                          ) |>
    set_engine("rpart") |>
    set_mode("regression")

# Fit
# registerDoParallel(cores = 4)
set.seed(69)
start <- Sys.time()
wflw_fit_tree <- workflow() %>% 
    add_model(model_spec_tree) %>% 
    add_recipe(recipe_spec %>% step_rm(date)) %>% 
    fit(training(splits))
end <- Sys.time()
end-start
# plan(sequential)

# Calibrate & Plot
calibrate_and_plot(
    wflw_fit_tree,
    type = 'testing',
    plot = FALSE
)


# * tuned bagged tree
# ** spec
model_spec_tree_tune <- bag_tree(
    tree_depth      = tune(), 
    min_n           = tune(), 
    cost_complexity = tune()
) |>
    set_engine("rpart") |>
    set_mode("regression")

# ** workflow
wflw_spec_tree_tune <- workflow() %>% 
    add_model(model_spec_tree_tune) %>% 
    add_recipe(recipe_spec %>% step_rm(date))

# ** Fit
set.seed(69)
start <- Sys.time()
tune_results_tree_tune <- wflw_spec_tree_tune %>% 
    tune_race_anova(
        resamples = resamples_kfold,
        grid = 6,
        control = control_race(verbose = T, parallel_over = NULL)
    )
end <- Sys.time()
end-start

# ** Results
tune_results_tree_tune %>% 
    show_best(metric = "rmse", n = Inf)

tune_results_tree_tune %>% 
    show_best(metric = "rsq", n = Inf)

# ** finalize
wflw_fit_tree_tune <- wflw_spec_tree_tune %>% 
    finalize_workflow(select_best(tune_results_tree_tune, metric = "rmse")) %>% 
    fit(training(splits))

modeltime_table(wflw_fit_tree_tune) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)
    ) %>% 
    plot_modeltime_forecast()

calibrate_and_plot(
    wflw_fit_tree_tune,
    type = 'testing',
    plot = FALSE
)

# save tune results
write_rds(wflw_fit_tree, "02_models/wflw_fit_tree.rds")
rm(wflw_fit_tree)
#rm(wflw_spec_tree) # no spec for rf tuning

gc()

# * Test stan ----
# basic fit
model_spec_stan <- linear_reg(
  mode = "regression",
) %>% 
  set_engine('stan') 


start <- Sys.time()
model_fit_stan <- workflow() %>% 
  add_model(model_spec_stan) %>% 
  add_recipe(recipe_spec %>% step_rm(date)) %>% 
  fit(training(splits))
end <- Sys.time()
end-start
# plan(sequential)

# Calibrate & Plot
calibrate_and_plot(
  model_fit_stan,
  type = 'testing',
  plot = T
)

# ** Results
augment(model_fit_stan,testing(splits) %>% 
            filter(symbol != "L")) %>% 
    rmse(.pred, Return_fwd_21)

# ** Tuning



start <- Sys.time()
set.seed(69)
tune_results_stan <- wflw_spec_stan_tune %>% 
  tune_grid(
    resamples = resamples_kfold,
    grid = 6,
    control = control_grid(verbose = T, parallel_over = NULL)
  )
end <- Sys.time()
end-start

# ** Results
tune_results_stan %>% 
  show_best(metric = "rmse", n = Inf)

tune_results_stan %>% 
  show_best(metric = "rsq", n = Inf)

# ** Finalize

wflw_fit_stan_tuned <- wflw_spec_stan_tune %>% 
  finalize_workflow(select_best(tune_results_stan, metric = "rmse")) %>% 
  fit(training(splits))

modeltime_table(wflw_fit_stan_tuned) %>% 
  modeltime_calibrate(new_data = testing(splits) %>% 
                        filter(symbol == testing_symbol)) %>% 
  modeltime_forecast(actual_data = data_prepared_tbl %>% 
                       filter(symbol == testing_symbol), 
                     new_data = testing(splits) %>% 
                       filter(symbol == testing_symbol)
  ) %>% 
  plot_modeltime_forecast()

calibrate_and_plot(wflw_fit_stan_tuned, plot =F)

# save tune results
write_rds(wflw_fit_stan, "02_models/wflw_fit_tree.rds")
rm(wflw_fit_tree)
#rm(wflw_spec_tree) # no spec for rf tuning

gc()

# * Test SVM ----

# * Test kNN ----

# * Test RNN ----
# https://kindling.joshuamarie.com/
# library(kindling)

model_spec_rnn <- mlp_kindling(
    mode = "regression",
    hidden_neurons = c(10, 10),
    # activations = act_funs(relu, relu),
    # optimizer = "adam",
    # learn_rate = 0.001,
    # penalty = 1e-4,
    # mixture = 0,
    # batch_size = 16,
    epochs = 100,
    validation_split = 0.2
    # verbose = FALSE
    # early_stopping = early_stop(patience = 10L, min_delta = 1e-3, monitor = "train_loss")
)

wflw_spec_rnn <- workflow() %>%
    add_recipe(recipe_spec %>% 
                   step_rm(date) %>% 
                   step_normalize(all_numeric_predictors())) %>%
    add_model(model_spec_rnn)

set.seed(101)
start   <- Sys.time()
fit_rnn <- wflw_spec_rnn %>% fit(train_short)
end     <- Sys.time()
end-start

rnn_pred <- augment(fit_rnn, new_data = testing(splits))

metrics(rnn_pred, truth = Return_fwd_21, estimate = .pred)

# * Test SAINT ----
# https://brulee.tidymodels.org/reference/brulee_saint.html

set.seed(101)
start <- Sys.time()
fit_saint <- brulee_saint(
    recipe_spec %>% 
        step_rm(date),
    data = train_short
)
end <- Sys.time()
end-start

augment(fit_nnet, testing(splits)) %>% 
    rmse(.pred, Return_fwd_21)

augment(fit_nnet, testing(splits)) %>% 
    metrics(.pred, Return_fwd_21)

# Modeling explanation ----
# * feature importance ----
extract_fit_engine(wflw_fit_lgb_tuned) %>%
  # extract_fit_engine(wflw_fit_lightgbm) %>%
  lightgbm::lgb.importance(model = .) %>% 
  as_tibble() %>% View()
  slice_max(order_by = Gain, n = 20) %>% 
  mutate(Feature = as_factor(Feature) %>% fct_rev()) %>% 
  ggplot(aes(x = Gain, y = Feature, fill = Feature))+
  geom_col(alpha = 0.7)+
  scale_fill_tq()+
  theme_few()+
  theme(legend.position = "none")+
  ggtitle("LightGBM Feature Importance")

extract_fit_engine(wflw_fit_xgboost_tuned) %>%
    xgboost::xgb.importance(model = .) %>%
    as_tibble() %>% View()
    slice_max(Gain, n = 20) %>% 
    mutate(Feature = as_factor(Feature) %>% fct_rev()) %>% 
    ggplot(aes(x = Gain, y = Feature, fill = Feature))+
    geom_col(alpha = 0.7)+
    scale_fill_tq()+
    theme_few()+
    theme(legend.position = "none")+
    ggtitle("XGBoost Feature Importance")

# extract_fit_engine(wflw_fit_xgboost_tuned) %>%
extract_fit_engine(wflw_fit_lgb_tuned) %>%
    lightgbm::lgb.importance(model = .) %>% 
    as_tibble() %>% 
  arrange(desc(Gain)) %>% 
  mutate(rank = row_number()) %>% #View()
  filter(str_detect(Feature, "date"))

# ** compare tree models ----
library(tidytext)
xgb_imp <- extract_fit_engine(wflw_fit_xgboost_tuned) %>%
    xgboost::xgb.importance(model = .) %>%
    as_tibble() %>% 
    filter(!str_detect(Feature,"symbol")) %>% 
    arrange(desc(Gain)) %>% 
    mutate(rank = row_number(),
           model = "xgboost")

lgb_imp <- extract_fit_engine(wflw_fit_lgb_tuned) %>%
    # extract_fit_engine(wflw_fit_lightgbm) %>%
    lightgbm::lgb.importance(model = .) %>% 
    as_tibble() %>% 
    filter(!str_detect(Feature,"symbol")) %>%
    arrange(desc(Gain)) %>% 
    mutate(rank = row_number(),
           model = "lightgbm")

cb_imp <- extract_fit_engine(wflw_fit_catboost) %>% 
    catboost::catboost.get_feature_importance(model = .) %>%
    as_tibble(rownames = 'Feature') %>% 
    rename(Gain = V1) %>% 
    filter(!str_detect(Feature,"symbol")) %>%
    arrange(desc(Gain)) %>% 
    mutate(rank = row_number(),
           model = "catboost")

bind_rows(xgb_imp,
          lgb_imp,
          cb_imp) %>% 
    group_by(model) %>% 
    #View()
    slice_max(Gain, n = 20) %>% 
    ungroup() %>% 
    ggplot(aes(x = Gain,
               y = reorder_within(x = Feature, by = Gain, within = model),
               fill = model)
    )+
    geom_col(show.legend = F)+
    facet_wrap(.~model, scales = 'free')+
    scale_y_reordered()+
    theme_bw()+
    ylab("")

bind_rows(xgb_imp, lgb_imp, cb_imp) %>% 
    select(-Cover, -Frequency, -Gain) %>% 
    filter(rank > 30) %>% 
    pivot_wider(names_from = model, values_from = rank) %>% View()

bind_rows(xgb_imp, lgb_imp, cb_imp) %>% 
    select(-Cover, -Frequency, -Gain) %>% 
    filter(rank <= 30) %>% 
    pivot_wider(names_from = model, values_from = rank) %>% View()

# extract names of top 25 Features by model
top_features <- bind_rows(xgb_imp,
          lgb_imp,
          cb_imp) %>% 
    filter(!str_detect(Feature, "_x_")) %>% # include this to remove the interaction terms to reduce the data before subsetting
    group_by(model) %>% 
    #View()
    slice_max(Gain, n = 25) %>% 
    ungroup() %>%
    pull(Feature) %>% 
    unique()

# ** Calibrate and show accuracy across models ----
multi_calibrate <- modeltime_table(
    wflw_fit_lgb_tuned,
    wflw_fit_xgboost_tuned
    ,wflw_fit_prophet_boost_tuned
    ,wflw_fit_cb
) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) 

multi_calibrate %>% 
    modeltime_forecast(actual_data = data_prepared_dt_filter %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)) %>% 
    plot_modeltime_forecast(.conf_interval_show = F)

multi_calibrate %>%
    modeltime_accuracy()

# scale_fill_viridis_d()

# ** SHAP Analysis ----
fit <- extract_fit_parsnip(wflw_fit_lgb_tuned)
fit <- extract_fit_parsnip(wflw_fit_xgboost_tuned)

fit_names <- fit[["preproc"]][["x_names"]]

df_explain  <- bake( 
    prep(recipe_spec), 
    has_role("predictor"),
    #new_data = training(splits)
    new_data = forecast_dt[symbol == "ADSK"]
    ) %>% 
    select(-date) %>% 
    select(all_of(fit_names))

shap_values <- extract_fit_engine(wflw_fit_lgb_tuned) %>% #fit %>% #
  shapviz(X_pred = data.matrix(df_explain), 
          X = df_explain)

# SHAP importance
shap_values %>% 
  sv_importance(show_numbers = TRUE, "beeswarm", alpha = 0.7) +
  ggtitle("SHAP importance")+
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
  theme_classic()

# python shap colors 
#c("#1E88E5", "#ff0d57", "#13B755", "#7C52FF", "#FFC000", "#00AEEF")

shap_values %>% 
  sv_importance(show_numbers = TRUE, fill = "#1e88e5", alpha = 0.7) +
  ggtitle("SHAP importance")+
  theme_classic()

shap_names <- shap_values %>% 
  sv_importance("no") %>% enframe() %>% arrange(desc(value)) %>% 
    mutate(rank = row_number()) 

shap_names %>% tail()

shap_values %>% 
    sv_dependence(shap_names$name[1], alpha = 0.7) +
    scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
    geom_hline(yintercept = 0)+
    geom_smooth(method = "gam")+
    theme_minimal()

shap_values %>% 
    sv_dependence(shap_names$name[2], alpha = 0.7) +
    scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
    geom_hline(yintercept = 0)+
    geom_smooth(method = "gam")+
    theme_minimal()

shap_values %>% 
  sv_dependence(shap_names$name[3], alpha = 0.7) +
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
    geom_hline(yintercept = 0)+
  geom_smooth(se = F,method = "gam",colour = "gray",linewidth = 1)+
  theme_minimal()

shap_values %>% 
  sv_dependence("Close_macd_long_signal_trend", alpha = 0.7) +
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
  geom_smooth(se = F,colour = "gray",linewidth = 1)+
  theme_minimal()

shap_values %>% 
  sv_dependence("Close_macd_short_signal_trend", alpha = 0.7) +
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
  geom_smooth(se = F,colour = "gray",linewidth = 1)+
  theme_minimal()

shap_values %>% 
  sv_dependence("Close_ratio_crude", alpha = 0.7) +
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
  theme_minimal()

xvars <- c("Vol_WAP_norm","Close_momentum_21_252_126","Close_natr_63","Close_rel_vol_14")
evars <- colnames(shap_values$S)[grep("eps", colnames(shap_values$S))]
dvars <- colnames(shap_values$S)[grep("div", colnames(shap_values$S))]
mvars <- colnames(shap_values$S %>% as_tibble() %>% select(contains("macd")))
vvars <- colnames(shap_values$S)[grep("WAP", colnames(shap_values$S))]
avars <- colnames(shap_values$S)[grep("ADX", colnames(shap_values$S))]

shap_values %>% 
  sv_dependence(vvars, viridis_args = list(option = "viridis", direction = -1))

shap_values %>% 
  sv_dependence(evars, viridis_args = list(option = "viridis", direction = -1))

shap_values %>% 
  sv_dependence(dvars, viridis_args = list(option = "viridis", direction = -1))

shap_values %>% 
    sv_dependence(mvars, viridis_args = list(option = "viridis", direction = -1))

shap_values %>% 
    sv_dependence(avars, viridis_args = list(option = "viridis", direction = -1))

shap_values %>% sv_waterfall(21)

# SHAP interactions for fwd return
shap_values %>%  
  sv_dependence("Vol_WAP_norm", 
                color_var = xvars, 
                interactions = TRUE,  
                viridis_args = list(option = "viridis", direction = -1))

shap_values %>% 
  sv_dependence2D("Vol_WAP_norm","Return_fwd_21", alpha = 0.7) +
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
  theme_minimal()


# 5.0 EVALUATE TUNED FORECASTS  -----
# * combined winrates ----
# list <- as.list(ls(pattern = '_winrate'))
winrates <- rbindlist(l = list(xgboost_winrate,
                               catboost_winrate,
                               prophet_winrate))

winrates[,.(rate = mean(win), total = sum(win)), keyby = model]

# * Model Table ----
submodels_tbl <- modeltime_table(
  wflw_fit_xgboost_tuned
  ,wflw_fit_lgb_tuned
  ,wflw_fit_prophet_boost_tuned
  ,wflw_fit_cb_short
  ,wflw_fit_glmnet_tuned
  ,wflw_fit_rf_final
) %>% 
    update_model_description(1, "XGBOOST - Tuned") %>% 
    update_model_description(2, "LightGBM - Tuned") %>% 
    update_model_description(3, "Prophet Boost - Tuned") %>% 
    update_model_description(4, "CatBoost") %>% 
    update_model_description(5, "GLMNet - Tuned") %>% 
    update_model_description(6, "RandomForest")


# * Calibration ----
calibration_tbl <- submodels_tbl %>% 
    modeltime_calibrate(testing(splits), id = "symbol")

# * Accuracy ----
calibration_tbl %>% 
    modeltime_accuracy(metric_set = extended_forecast_accuracy_metric_set()) %>% 
    arrange(rmse)

# * Accuracy summary plot ----
calibration_tbl %>% 
  modeltime_accuracy(acc_by_id = T) %>% #View()
  #arrange(rmse) %>% 
  select(.model_desc, rmse:rsq) %>% #group_by(.model_desc) %>% get_summary_stats()
  pivot_longer(-.model_desc) %>% 
  ggplot(aes(x = .model_desc, y = value, fill = .model_desc))+
  geom_boxplot(alpha = 0.7, show.legend = F) +
  facet_grid(name~., scales = "free_y")+
    scale_fill_tq()+
  theme_tq()

# * Accuracy by symbol ----
acc_by_symbol <- calibration_tbl %>% 
    modeltime_accuracy(acc_by_id = TRUE) %>% 
    select(-.type) %>% 
    pivot_longer(-(.model_id:symbol)) %>% 
    group_by(symbol, name) %>%
    summarise(value = mean(value)) %>% 
    ungroup() %>% 
    pivot_wider(names_from = name, values_from = value)

# write_rds(acc_by_symbol, str_glue("02_models/{today()}_acc_by_symbol.rds"))

# * Forecast symbols ----
# extract the subset of symbols that have the lowest rmse
# can do either a fixed n or a percentage

forecast_symbols <- acc_by_symbol %>% 
    slice_min(rmse, n = 200) %>%
    #slice_min(rmse, prop = 0.4) %>%
    droplevels() %>% 
    arrange(symbol) %>% 
    pull(symbol)

# * Forecast Test ----
gc()
forecast_test <- calibration_tbl %>% 
    modeltime_forecast(
        new_data   = testing(splits),
        actual_data = data_prepared_dt_short[!is.na(Return_fwd_21)],#data_prepared_tbl,
        keep_data  = T, # keeps the grouping variable and base data
        conf_by_id = T) 

forecast_test %>% plot_fcst()

forecast_test %>% filter(symbol == "AAPL") %>% plot_modeltime_forecast(.conf_interval_show = F)

# * backtesting ----
back_fcst <- calibration_tbl %>% 
    modeltime_forecast(
        new_data = data_prepared_dt_short %>% 
            filter(date %between% c("2025-12-01","2025-12-10")) %>% 
            drop_na(),
        actual_data = data_prepared_dt_short %>% 
            filter(date %between% c("2025-11-01","2025-12-10")) %>% 
            drop_na(),
        keep_data = T,
        conf_by_id = T
    ) 

back_fcst %>% 
    filter(date == "2025-12-08" ) %>% 
    arrange(desc(.value))

backtest <- back_fcst %>% 
    filter(date >= "2025-12-01") %>% 
    select(symbol, .value, .model_desc, date) %>% 
    pivot_wider(names_from = .model_desc, values_from = .value, 
                names_repair = "universal") %>%
    group_by(symbol, date) %>% 
    mutate(forecast_mean = mean(c_across(c(contains("Tuned"))), na.rm=TRUE),
           forecast_median = median(c_across(c(contains("Tuned"))), na.rm=TRUE)) %>% 
    ungroup() %>% 
    mutate(correct_mean = if_else((forecast_median < 0 & ACTUAL < 0)|
                               (forecast_median >= 0 & ACTUAL >= 0),1,0),
           correct_median = if_else((forecast_median < 0 & ACTUAL < 0)|
                                      (forecast_median >= 0 & ACTUAL >= 0),1,0)) 

backtest %>% View()
rmse(backtest, truth = ACTUAL, estimate = forecast_median)
rmse(backtest, truth = ACTUAL, estimate = forecast_mean)
    #filter(date == "2024-12-06") 

#symbol %in% c("NVDA","GE","FI","FIS","CTAS","WELL","MSFT","MMM","IRM","NVDA","TT")

back_fcst %>% 
    plot_modeltime_forecast(.facet_vars = symbol, .facet_ncol = 4)

# ** forecast test plot ----
forecast_test %>% 
    group_by(symbol) %>%
    #filter(symbol == "AAP") %>% 
    # filter(symbol %in% c("NVDA","GE","FI","FIS","CTAS","WELL","MSFT","MMM","IRM")) %>% 
    filter(.index >= "2025-08-01") %>% 
    plot_modeltime_forecast(
        .facet_vars = symbol,
        .facet_ncol = 2,
        .conf_interval_show = F,
        .interactive = T,
        .trelliscope = T
    )

test_fcst <- forecast_test %>% 
  filter(.index == max(.index) & symbol %in% c("NVDA","GE","FI","FIS","CTAS","WELL","CRWD")) %>% 
  select(.model_id, .model_desc, symbol, date, .value, .conf_lo, .conf_hi)

test_fcst %>% filter(!is.na(.model_id)) %>% group_by(symbol) %>% summarise(mean = mean(.value), median = median(.value))
test_fcst %>% filter(is.na(.model_id))

# 6.0 RESAMPLING ----
# - Assess the stability of our models over time
# - Helps us strategize an ensemble approach

# * Time Series CV ----

resamples_tscv <- training(splits) %>% 
    time_series_cv(
        initial     = 252,
        assess      = 21,
        skip        = 21,
        cumulative  = T,
        slice_limit = 12
    )

resamples_tscv %>% 
    tk_time_series_cv_plan() %>% 
    plot_time_series_cv_plan(date, Return_fwd_21,
                             .facet_ncol = 3,
                             .interactive = F)
  

# * Fitting Resamples ----

model_tbl_tuned_resamples <- submodels_tbl %>% 
  #modeltime_table(wflw_fit_lgb_tuned) %>% 
    modeltime_fit_resamples(
        resamples = resamples_tscv,
        control   = control_resamples(verbose = T, parallel_over = NULL)
    )

# * Resampling Accuracy Table ----

model_tbl_tuned_resamples %>% 
    modeltime_resample_accuracy(
        metric_set = metric_set(mae, rmse, rsq),
        summary_fns = list(mean = mean, sd = sd)
    ) %>% 
    arrange(rmse_mean)

model_tbl_tuned_resamples %>% 
  modeltime_resample_accuracy(
    metric_set = metric_set(rmse, rsq),
    summary_fns = NULL
  ) 

# * Resampling Accuracy Plot ----
model_tbl_tuned_resamples %>% 
    plot_modeltime_resamples(
        .metric_set = metric_set(rmse, rsq),
        .point_size = 2,
        .point_alpha = 0.8,
        .facet_ncol = 1
    )

model_tbl_tuned_resamples %>% 
    modeltime_resample_accuracy(
        metric_set = metric_set(rmse, rsq),
        summary_fns = NULL
    ) %>% 
    select(.model_desc, rmse, rsq) %>% 
    pivot_longer(-.model_desc) %>% 
    ggplot(aes(x = .model_desc, y = value, fill = .model_desc))+
    geom_boxplot(alpha = 0.7, show.legend = F) +
    facet_grid(name~., scales = "free_y")+
    scale_fill_tq()+
    theme_tq()

# 7.0 ENSEMBLE PANEL MODELS -----

# * Average Ensemble ----
# submodels_ids_to_keep <- c(1,3,4)

# ensemble_fit <- submodels_tbl %>% 
#     #filter(.model_id %in% submodels_ids_to_keep) %>% 
#     ensemble_average(type = 'mean')

ensemble_fit <- calibration_tbl %>%
    ensemble_average(type = 'median') # reduces effects of bad forecasts and reduces overfitting. probably not needed with only 3 forecasts

model_ensemble_tbl <- modeltime_table(ensemble_fit)

# * Accuracy ----
model_ensemble_tbl %>% 
    modeltime_calibrate(testing(splits),
                        id = "symbol") %>% 
  modeltime_accuracy(testing(splits),
                     metric_set = extended_forecast_accuracy_metric_set())

# * Weighted ensemble ----
loadings_tbl <- submodels_tbl %>% 
  #filter(.model_id %in% submodels_ids_to_keep) %>% 
  modeltime_accuracy(testing(splits)) %>%
  mutate(rank = min_rank(-rmse))

ensemble_fit_wt <- submodels_tbl %>%
  ensemble_weighted(loadings = loadings_tbl$rank)

ensemble_fit_wt$fit$loadings_tbl

# * Calibrate weighted ensemble ----

model_ensemble_tbl_wt <- modeltime_table(
  ensemble_fit_wt
) %>% 
    modeltime_calibrate(testing(splits),
                        id = "symbol")

model_ensemble_tbl_wt %>%
  modeltime_accuracy(testing(splits), 
                     metric_set = extended_forecast_accuracy_metric_set())

write_rds(model_ensemble_tbl_wt, 
          str_glue("02_models/{today()}_model_ensemble_tbl_wt.rds"),
          compress = "gz")

# * lm stack Test ----
# linear regression stacking does not seem to offer much improvement if any
# and the computation on the prophet boosting is very intensive

# ** refit samples ----
submodels_resamples_kfold_tbl <- submodels_tbl %>% 
 #filter(.model_id %in% c(1:3)) %>% 
  modeltime_fit_resamples(
    resamples       = resamples_kfold_short,
    control         = control_resamples(
      verbose       = T,
      parallel_over = NULL
    )
  )

# ** workaround if prophet boost fails ----
prophet_boost_resamples_tbl <- modeltime_table(
  wflw_fit_prophet_boost_tuned
  ) %>%  
  modeltime_fit_resamples(
    resamples = resamples_kfold,
    control   = control_resamples(
      verbose   = T,
      parallel_over = "everything"
      )
    )

submodels_resamples_kfold_tbl_c <- combine_modeltime_tables(submodels_resamples_kfold_tbl %>% 
                                                              filter(.model_id %in% c(1:3)), 
                                                            prophet_boost_resamples_tbl)

submodels_resamples_kfold_tbl_cal <- submodels_resamples_kfold_tbl %>%
    modeltime_calibrate(testing(splits), 
                        id = "symbol")


# ** fit lm ----
ensemble_fit_lm <- submodels_resamples_kfold_tbl %>% 
  ensemble_model_spec(
    model_spec = linear_reg() %>% set_engine("lm"),
    control    = control_grid(verbose = T)
  )

# test accuracy
modeltime_table(
  ensemble_fit_lm
) %>% 
  modeltime_accuracy(testing(splits_short))

# 8.0 Ensemble Forecast test ----
# * Ensemble forecast
gc()
forecast_ensemble_test_tbl <- model_ensemble_tbl_wt %>% 
                              #model_ensemble_tbl %>%
    modeltime_forecast(
        new_data    = testing(splits),
        actual_data = data_prepared_dt_short[!is.na(Return_fwd_21)],#data_prepared_dt_short,
        keep_data   = T
    )

# * ensemble forecast test plot ----
forecast_ensemble_test_tbl %>% 
    group_by(symbol) %>% 
    plot_modeltime_forecast(
        .facet_ncol = 3,
        .conf_interval_show = F,
        .interactive = T,
        .legend_show = F,
        .trelliscope = T
    )

# * ensemble forecast test table ----
forecast_ensemble_test_tbl %>% 
    filter(.key == 'prediction') %>% 
    select(symbol, .value, Return_fwd_21) %>% 
    #group_by(symbol) %>% 
    summarize_accuracy_metrics(
        truth = Return_fwd_21,
        estimate = .value,
        metric_set = metric_set(mae, rmse, rsq)
    )

# 9.0 Final Forecasting ----
# * Refit ----
data_prepared_clean_tbl <- data_prepared_tbl %>% 
    #filter_by_time(date, .start = max(date)-years(2)) %>% 
    #filter(symbol %in% forecast_symbols) %>% 
    #droplevels() %>% 
    drop_na()

model_ensemble_refit_tbl <- model_ensemble_tbl_wt %>% 
    modeltime_refit(data_prepared_clean_tbl)

# * Ensemble final forecast ----
final_forecast_tbl <- forecast_tbl %>% filter(symbol %in% forecast_symbols)

model_ensemble_final_forecast <- model_ensemble_refit_tbl %>% 
    modeltime_forecast(
        new_data    = final_forecast_tbl,
        actual_data = data_prepared_clean_tbl,
        keep_data   = T,
        conf_by_id  = T
    )  

# * plot final forecast ----
model_ensemble_final_forecast %>% 
  arrange(desc(date)) %>% 
    group_by(symbol) %>% 
    plot_modeltime_forecast(
        .facet_ncol = 2,
        .y_intercept = 0,
        .conf_interval_show = F,
        .legend_show = F,
        .trelliscope = F
    )

model_ensemble_final_forecast %>% plot_fcst(scales = "free_y", ncol = 4)

# * View final forecasts ----
model_ensemble_final_forecast %>% 
    filter(date == max(date)) %>% 
    select(symbol, .value, date) %>% 
    arrange(desc(.value)) %>% 
    left_join(acc_by_symbol) %>% View()

model_ensemble_final_forecast %>% 
    filter(date == max(date)) %>% 
    select(symbol, .value, date) %>% 
    arrange(desc(.value)) %>% 
    left_join(acc_by_symbol) %>%
    filter(.value >= 0) %>% 
    #slice_min(rmse, n = 30) %>% 
    filter(rmse < 0.07) %>% 
    arrange(desc(.value))

# * Turn OFF Parallel Backend ----
plan(sequential)
parallel_stop()

# * analysis of forecasts ----
model_ensemble_final_forecast %>% 
    filter(.key == "prediction") %>% 
    select(where(is.numeric),
           -.model_id, 
           -rowid, 
           -starts_with("return"),
           -contains(".conf")) %>% 
    cor_test(.value) %>% 
    mutate(var2 = as.factor(var2) %>% fct_reorder(abs(cor))) %>% 
    plot_ly(x = ~cor, y = ~var2, type = "bar")

model_ensemble_final_forecast %>% 
    filter(date == max(date)) %>% 
    select(symbol, .value, date, Vol_WAP_norm) %>% 
    arrange(desc(.value)) %>% print(n = 20)

model_ensemble_final_forecast %>% 
    filter(date == max(date)) %>% 
    select(symbol, .value, .conf_lo, .conf_hi) %>% 
    arrange(desc(.value))

model_ensemble_final_forecast %>% 
    filter(date == max(date)) %>% 
    select(symbol, .value, .conf_lo, .conf_hi) %>% 
    arrange(desc(.conf_lo))

model_ensemble_final_forecast %>% 
    filter(date == max(date)) %>% 
    select(symbol, .value, .conf_lo, .conf_hi) %>% 
    arrange(desc(.conf_hi))

# 10.0 save forecasts ----
write_rds(model_ensemble_final_forecast,
          str_glue("01_save_data/01_saved_forecasts/{today()}_model_ensemble_final_forecast.rds"))



# Testing for the tidymodels forecasting ----
# instead of stacking can we estimate the model accuracy each from the augment()
# use these weights to then estimate the forecast?

# acc on testing
lgb_test <- augment(wflw_fit_lgb_tuned, test) %>% mutate(model = "lightgbm") %>% select(symbol, model, date, .pred, actual = Return_fwd_21)
xgb_test <- augment(wflw_fit_xgboost_tuned, test) %>% mutate(model = "xgboost") %>% select(symbol, model, date, .pred, actual = Return_fwd_21)
cb_test  <- augment(wflw_fit_catboost, test) %>% mutate(model = "catboost") %>% select(symbol, model, date, .pred, actual = Return_fwd_21)
pb_test  <- augment(wflw_fit_prophet_boost_tuned, test) %>% mutate(model = "prophet_xgb") %>% select(symbol, model, date, .pred, actual = Return_fwd_21)
glm_test <- augment(wflw_fit_glmnet_tuned, test) %>% mutate(model = "glmnet") %>% select(symbol, model, date, .pred, actual = Return_fwd_21)

test_combined <- rbind(lgb_test
                      ,xgb_test
                      ,cb_test
                      ,pb_test
                      ,glm_test) %>% setDT()

acc_by_symbol <- test_combined[,.(rmse = sqrt(mean((.pred-actual)^2))), keyby = symbol]

acc_symbols <- acc_by_symbol %>% 
    slice_min(rmse, prop = 0.6) %>% 
    pull(symbol)

# weighting
winrates[,.(rate = mean(win),
            total = sum(win),
            gainers = mean(actual_gain), 
            lift = mean(win)/mean(actual_gain)), 
         keyby = model]

weights <- test_combined[,.(rmse = sqrt(mean((.pred-actual)^2))), 
              keyby = model][,weight := frank(-rmse)/sum(frank(rmse))]
setorder(weights, rmse)
print(weights)

# combined forecasts
lgb_fcst <- augment(wflw_fit_lgb_tuned, forecast_dt) %>% mutate(model = "lightgbm") %>% select(symbol, model, date, .pred)
xgb_fcst <- augment(wflw_fit_xgboost_tuned, forecast_dt) %>% mutate(model = "xgboost") %>% select(symbol, model, date, .pred)
cb_fcst  <- augment(wflw_fit_lgb_tuned, forecast_dt) %>% mutate(model = "catboost") %>% select(symbol, model, date, .pred)
pb_fcst  <- augment(wflw_fit_lgb_tuned, forecast_dt) %>% mutate(model = "prophet_xgb") %>% select(symbol, model, date, .pred)
glm_fcst <- augment(wflw_fit_lgb_tuned, forecast_dt) %>% mutate(model = "glmnet") %>% select(symbol, model, date, .pred)

combined_fcst <- rbind(lgb_fcst, xgb_fcst, cb_fcst, pb_fcst, glm_fcst)
combined_fcst %>% count(model)

merge_fcst_weight      <- setDT(merge(combined_fcst, weights[,.(model, weight)]))
combined_weighted_fcst <- merge_fcst_weight[,.(fcst = sum(.pred*weight)/sum(weight)),symbol]

combined_weighted_fcst %>% 
    filter(symbol %in% acc_symbols) %>% 
    slice_max(fcst, n = 10)

stock_picks            <- combined_weighted_fcst %>% 
    filter(symbol %in% acc_symbols) %>% 
    slice_max(fcst, n = 10) %>% 
    pull(symbol)
