...
add_best_features <- function(prices_dt, price) {
  ...
  prices_dt[, Close_macd_long := MACD(price_col, 50, 200, 30, maType = "EMA")[, "macd"]]
  prices_dt[, Close_macd_long_signal := MACD(price_col, 50, 200, 30, maType = "EMA")[, "signal"]]
  prices_dt[, Close_macd_short := MACD(price_col, 14, 30, 9, list(list(EMA, wilder=TRUE),list(EMA, wilder=TRUE),list(EMA, wilder=TRUE)))[, "macd"]]
  prices_dt[, Close_macd_short_signal := MACD(price_col, 14, 30, 9, list(list(EMA, wilder=TRUE),list(EMA, wilder=TRUE),list(EMA, wilder=TRUE)))[, "signal"]]
  prices_dt[, ":=" (
...
prices_features_tbl[ , .SD[which.max(Return_fwd_21)], by = symbol]
...
# creates additional indicators for earnings
...
prices_earnings_tbl %>% 
    filter(symbol == "AAPL" & date == "2022-09-12")
...
spy_dt[, spy_state := hidden_markov(.SD), by = symbol, .SDcols = c("date", "close")]
end <- Sys.time()
end-start
...
spy_dt %>% group_by(spy_state) %>% get_summary_stats(return_spy_21)
...
# * review returns by SPY state ----
prices_earnings_tbl %>% 
    group_by(symbol) %>% 
    mutate(Return_fwd_21_log = ROC(close, n = 21, 'continuous')) %>% 
    ungroup() %>% 
    group_by(spy_state) %>% 
    summarise(return = sum(Return_fwd_21_log, na.rm = T)) %>% 
    ungroup() %>% 
    ggplot(aes(x = return, y = symbol, fill = as.factor(spy_state)))+
    geom_col()+
    facet_wrap(~spy_state, scales = 'free')
...
prices_earnings_econ_tbl %>% 
  select(Return_fwd_21, eps_surprise, eps_growth, 
         value_UNRATE, value_FEDFUNDS, unrate_over) %>% 
  cor_test() %>% 
  filter(var1 != var2) %>% 
  plot_ly(z = ~cor,
          x = ~var1,
...
prices_earnings_econ_tbl %>% 
  ...
  plot_ly(x = ~date,
          y = ~Return_fwd_21,
          color = ~as.factor(spy_state),
          type = "scatter")

prices_earnings_econ_tbl[symbol == "NVDA",] %>% 
    group_by(spy_state) %>% 
    get_summary_stats(Return_fwd_21)

prices_earnings_econ_tbl %>% 
    filter(symbol == "NVDA") %>% 
    plot_ly(x = ~Close_macd_long_trend,
            y = ~Return_fwd_21,
            color = ~as.factor(unrate_over),
            type = "scatter")
...
prices_earnings_tbl %>% 
    plot_acf_diagnostics(date, close, .ccf_vars = Vol_WAP_norm, .show_ccf_vars_only = T)
...
full_data <- prices_earnings_econ_tbl %>% 
  ...
  group_by(symbol) %>% 
  arrange(date) %>% 
  tk_augment_fourier(date, .periods = c(63,252), .K = 2) %>%
...
full_data %>% 
  select(starts_with("Close_"), return_spy_21, Return_fwd_21) %>% 
  drop_na() %>% 
  cor_test() %>% #View()
  plot_ly(x = ~var1,
          y = ~var2,
...
# rm(prices_base)
# not rm(prices_dt) since we can use this for the optimization and/or backtesting
gc()
...
recipe_spec %>% prep() %>% bake(new_data = train_cleaned %>% slice_sample(prop = 0.1)) %>% glimpse()
...
options(future.globals.maxSize = 2.1 * 1024^3)  # Set to 1.5 GiB
...
# plan(strategy = cluster,  workers = 3)

# plan(multisession, workers = 4)

parallel_start(3, .method = "future")
...
write_rds(tune_results_xgboost, "02_models/tune_results_xgboost.rds")
...
wflw_spec_glmnet_tune <- workflow() %>% 
  ...
  add_recipe(recipe_spec %>% step_rm(date) %>% step_naomit(all_numeric_predictors()))
...
# Calibrate & Plot
modeltime_table(wflw_fit_lm) %>% 
    modeltime_accuracy(new_data = testing(splits), id = "symbol")

modeltime_table(wflw_fit_lm) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% filter(symbol == testing_symbol)) %>% 
    modeltime_forecast(
        actual_data = data_prepared_tbl %>% filter(symbol == testing_symbol), 
        new_data    = testing(splits) %>% filter(symbol == testing_symbol),
        keep_data   = T) %>% 
    plot_modeltime_forecast()
...
modeltime_table(wflw_fit_nnet) %>% 
    modeltime_calibrate(new_data = testing(splits) %>% 
                            filter(symbol == testing_symbol)) %>% modeltime_accuracy(metric_set = extended_forecast_accuracy_metric_set())
    modeltime_forecast(actual_data = data_prepared_tbl %>% 
                           filter(symbol == testing_symbol), 
                       new_data = testing(splits) %>% 
                           filter(symbol == testing_symbol)) %>% 
...
extract_fit_engine(wflw_fit_xgboost_tuned) %>%
  ...
  as_tibble() %>% #View()
  slice_max(Gain, n = 20) %>% #print(n = nrow(.))
...
extract_fit_engine(wflw_fit_lgb_tuned) %>%
  ...
  as_tibble() %>%
  arrange(desc(Gain)) %>% 
  mutate(rank = row_number()) %>% print(n = nrow(.))
...
df_explain  <- bake( 
  prep(recipe_spec), 
  has_role("predictor"),
  new_data = train_cleaned %>% slice_sample(prop = 0.3)
  #composition = "matrix"
  ) %>% 
  select(-date)
...
shap_values %>% 
  sv_dependence("spy_state", alpha = 0.7) +
  scale_color_gradient(high = "#ff0d57", low = "#1e88e5")+
  #geom_smooth(se = F,colour = "gray",linewidth = 1)+
  theme_minimal()
...
# * Accuracy by forecast ----
acc_by_model <- calibration_tbl %>% 
...
# * test with best accuracy symbols ---
forecast_ensemble_test_tbl_filter <- model_ensemble_tbl_wt %>% 
    modeltime_forecast(
        new_data = testing(splits) %>% filter(symbol %in% forecast_symbols),
        actual_data = data_prepared_tbl %>% filter(symbol %in% forecast_symbols),
        keep_data = T
    )

forecast_ensemble_test_tbl_filter %>% 
    group_by(symbol) %>% 
    plot_modeltime_forecast(
        .facet_ncol = 2,
        .conf_interval_show = F,
...
# * Refit ----
data_prepared_filter_tbl <- data_prepared_tbl %>% 
...
model_ensemble_refit_tbl <- model_ensemble_tbl_wt %>% 
    modeltime_refit(data_prepared_filter_tbl)

# * Ensemble final forecast ----
model_ensemble_final_forecast <- model_ensemble_refit_tbl %>% 
    modeltime_forecast(
        new_data    = forecast_tbl %>% filter(symbol %in% forecast_symbols),
        actual_data = data_prepared_filter_tbl,
        keep_data = T
    )  
...
write_rds(model_ensemble_final_forecast,
          str_glue("01_save_data/{today()}_model_ensemble_final_forecast.rds"))
