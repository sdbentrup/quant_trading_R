...
prices_base <- tq_get(unique(holdings), from = from) %>% 
...
write_rds(full_data, str_glue("01_save_data/{today()}_portfolio_full_data.rds"))
...
parallel_start(4, .method = "future")
...
df_explain  <- bake( 
  prep(recipe_spec), 
  has_role("predictor"),
  new_data = train_cleaned %>% slice_sample(prop = 0.6)
  #composition = "matrix"
  ) %>% 
  select(-date)
...
submodels_tbl <- modeltime_table(
    ...
    update_model_description(5, "Ranger RF")
...
forecast_ensemble_test_tbl_filter <- model_ensemble_tbl %>% 
    modeltime_forecast(
        new_data = testing(splits),
        actual_data = data_prepared_tbl,
        keep_data = T
